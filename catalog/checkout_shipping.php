...
// load all enabled shipping modules
  ...
  if ( defined('MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING') && (MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING == 'true') ) {
    ...
    if ( ($pass == true) && ($order->info['total'] >= MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER) ) {
      ...
      $KUUZU_Language->loadDefinitions('modules/order_total/ot_shipping');
    }
  } else {
    $free_shipping = false;
  }
  ...
  if ( isset($_POST['action']) && ($_POST['action'] == 'process') && isset($_POST['formid']) && ($_POST['formid'] == $_SESSION['sessiontoken']) ) {
    ...
    if ( (tep_count_shipping_modules() > 0) || ($free_shipping == true) ) {
      if ( (isset($_POST['shipping'])) && (strpos($_POST['shipping'], '_')) ) {
        ...
        if ( isset($KUUZU_SM) || ($_SESSION['shipping'] == 'free_free') ) {
          if ($_SESSION['shipping'] == 'free_free') {
            $quote[0]['methods'][0]['title'] = KUUZU::getDef('free_shipping_title');
...
// method if more than one module is now enabled
  ...
  $KUUZU_Language->loadDefinitions('checkout_shipping');
  ...
  if ( defined('SHIPPING_ALLOW_UNDEFINED_ZONES') && (SHIPPING_ALLOW_UNDEFINED_ZONES == 'False') && (!isset($_SESSION['shipping']) || ($_SESSION['shipping'] === false)) ) {
    $messageStack->add_session('checkout_address', KUUZU::getDef('error_no_shipping_available_to_shipping_address'));

    KUUZU::redirect('checkout_shipping_address.php');
  }

  $breadcrumb->add(KUUZU::getDef('navbar_title_1'), KUUZU::link('checkout_shipping.php'));
  $breadcrumb->add(KUUZU::getDef('navbar_title_2'), KUUZU::link('checkout_shipping.php'));
...
<?php echo HTML::form('checkout_address', KUUZU::link('checkout_shipping.php'), 'post', 'class="form-horizontal"', ['tokenize' => true, 'action' => 'process']); ?>
  ...
  <div class="contentText row">
    <div class="col-sm-8">
      <div class="alert alert-warning">
        <?php echo KUUZU::getDef('text_choose_shipping_destination'); ?>
        <div class="clearfix"></div>
        <div class="pull-right">
          <?php echo HTML::button(KUUZU::getDef('image_button_change_address'), 'fa fa-home', KUUZU::link('checkout_shipping_address.php')); ?>
        </div>
        <div class="clearfix"></div>
      </div>
    </div>
...
?>
  ...
  <h2><?php echo KUUZU::getDef('table_heading_shipping_method'); ?></h2>
...
?>
  ...
  <div class="contentText">
    <div class="alert alert-warning">
      <div class="row">
        <div class="col-xs-8">
          <?php echo KUUZU::getDef('text_choose_shipping_method'); ?>
        </div>
...
?>
  ...
  <div class="contentText">
    <div class="alert alert-info"><?php echo KUUZU::getDef('text_enter_shipping_information'); ?></div>
  </div>
...
?>
    ...
    <div class="contentText">
      <div class="panel panel-success">
        <div class="panel-heading"><strong><?= KUUZU::getDef('free_shipping_title'); ?></strong>&nbsp;<?php echo $quotes[$i]['icon']; ?></div>
        <div class="panel-body">
          <?php echo KUUZU::getDef('free_shipping_description', ['amount' => $currencies->format(MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER)]) . HTML::hiddenField('shipping', 'free_free'); ?>
        </div>
      </div>
    </div>
