...
// Check that order_id, customer_id and filename match
  ...
  $Qdownload->bindInt(':language_id', $KUUZU_Language->getId());
  $Qdownload->execute();
...
// Die if file is not there
  if (!is_file(KUUZU::getConfig('dir_root') . 'download/' . $Qdownload->value('orders_products_filename'))) die;

// Now decrement counter
  $Qupdate = $KUUZU_Db->prepare('update :table_orders_products_download set download_count = download_count-1 where orders_products_download_id = :orders_products_download_id');
...
// This will work only on Unix/Linux hosts
    tep_unlink_temp_dir(KUUZU::getConfig('dir_root') . 'pub/');
    $tempdir = tep_random_name();
    umask(0000);
    mkdir(KUUZU::getConfig('dir_root') . 'pub/' . $tempdir, 0777);
    symlink(KUUZU::getConfig('dir_root') . 'download/' . $Qdownload->value('orders_products_filename'), KUUZU::getConfig('dir_root', 'Shop') . 'pub/' . $tempdir . '/' . $Qdownload->value('orders_products_filename'));
    if (is_file(KUUZU::getConfig('dir_root') . 'pub/' . $tempdir . '/' . $Qdownload->value('orders_products_filename'))) {
      KUUZU::redirect('pub/' . $tempdir . '/' . $Qdownload->value('orders_products_filename'));
    }
  }

// Fallback to readfile() delivery method. This will work on all systems, but will need considerable resources
  readfile(KUUZU::getConfig('dir_root') . 'download/' . $Qdownload->value('orders_products_filename'));
?>
