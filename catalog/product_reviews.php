...
/**
  ...
  */
  ...
  $KUUZU_Language->loadDefinitions('product_reviews');
...
<?php
  $Qa = $KUUZU_Db->prepare('select AVG(r.reviews_rating) as average, COUNT(r.reviews_rating) as count from :table_reviews r left join :table_reviews_description rd on r.reviews_id = rd.reviews_id where r.products_id = :products_id and r.reviews_status = 1 and rd.languages_id = :languages_id');
  $Qa->bindInt(':products_id', $Qcheck->valueInt('products_id'));
  $Qa->bindInt(':languages_id', $KUUZU_Language->getId());
//  $Qa->setCache('product_reviews_avg-' . $KUUZU_Language->get('code') . '-p' . $Qcheck->valueInt('products_id'));
  $Qa->execute();
  ...
  echo    KUUZU::getDef('reviews_text_average', ['count' =>  $Qa->valueInt('count'), 'average' =>  HTML::stars(round($Qa->value('average')))]);
  echo '</div>';
?>
...
?>
  ...
  <div class="col-sm-4 text-center">
    ...
    <p><?php echo HTML::button(KUUZU::getDef('image_button_in_cart'), 'fa fa-shopping-cart', KUUZU::link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now')); ?></p>
  </div>
...
<div class="row">
  <div class="col-sm-6 pagenumber hidden-xs">
    <?php echo $Qreviews->getPageSetLabel(KUUZU::getDef('text_display_number_of_reviews')); ?>
  </div>
  <div class="col-sm-6">
...
?>
      <blockquote class="col-sm-6" itemprop="review" itemscope itemtype="http://schema.org/Review">
        ...
        echo KUUZU::getDef('reviews_text_rated', [
...
?>
  ...
  <div class="alert alert-info">
    <?php echo KUUZU::getDef('text_no_reviews'); ?>
  </div>
  ...
  <div class="buttonSet row">
    <div class="col-xs-6">
      ...
      if (isset($_SESSION['navigation']->path[$back])) {
        echo HTML::button(KUUZU::getDef('image_button_back'), 'fa fa-angle-left', KUUZU::link($_SESSION['navigation']->path[$back]['page'], tep_array_to_string($_SESSION['navigation']->path[$back]['get'], array('action'))));
      }
      ?>&nbsp;
    </div>
    <div class="col-xs-6 text-right"><?php echo HTML::button(KUUZU::getDef('image_button_write_review'), 'fa fa-commenting', KUUZU::link('product_reviews_write.php', tep_get_all_get_params()), null, null, 'btn-success btn-reviews btn-buy'); ?></div>
  </div>
</div>
