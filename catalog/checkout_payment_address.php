...
// if there is nothing in the customers cart, redirect them to the shopping cart page
  ...
  $KUUZU_Language->loadDefinitions('checkout_payment_address');
    ...
    if (tep_not_null($_POST['firstname']) && tep_not_null($_POST['lastname']) && tep_not_null($_POST['street_address'])) {
      ...
      if (ACCOUNT_GENDER == 'true') {
        if ( ($gender != 'm') && ($gender != 'f') ) {
          ...
          $messageStack->add('checkout_address', KUUZU::getDef('entry_gender_error'));
        }
      }
      ...
      if (strlen($firstname) < ENTRY_FIRST_NAME_MIN_LENGTH) {
        ...
        $messageStack->add('checkout_address', KUUZU::getDef('entry_first_name_error', ['min_length' => ENTRY_FIRST_NAME_MIN_LENGTH]));
      }

      if (strlen($lastname) < ENTRY_LAST_NAME_MIN_LENGTH) {
        $error = true;

        $messageStack->add('checkout_address', KUUZU::getDef('entry_last_name_error', ['min_length' => ENTRY_LAST_NAME_MIN_LENGTH]));
      }

      if (strlen($street_address) < ENTRY_STREET_ADDRESS_MIN_LENGTH) {
        $error = true;

        $messageStack->add('checkout_address', KUUZU::getDef('entry_street_address_error', ['min_length' => ENTRY_STREET_ADDRESS_MIN_LENGTH]));
      }

      if (strlen($postcode) < ENTRY_POSTCODE_MIN_LENGTH) {
        $error = true;

        $messageStack->add('checkout_address', KUUZU::getDef('entry_post_code_error', ['min_length' => ENTRY_POSTCODE_MIN_LENGTH]));
      }

      if (strlen($city) < ENTRY_CITY_MIN_LENGTH) {
        $error = true;

        $messageStack->add('checkout_address', KUUZU::getDef('entry_city_error', ['min_length' => ENTRY_CITY_MIN_LENGTH]));
      }
      ...
      if (ACCOUNT_STATE == 'true') {
        ...
        if ($entry_state_has_zones == true) {
          ...
          if (count($Qzone->fetchAll()) === 1) {
            ...
            $messageStack->add('checkout_address', KUUZU::getDef('entry_state_error_select'));
          }
        } else {
          if (strlen($state) < ENTRY_STATE_MIN_LENGTH) {
            $error = true;

            $messageStack->add('checkout_address', KUUZU::getDef('entry_state_error', ['min_length' => ENTRY_STATE_MIN_LENGTH]));
          }
        }
      }
      ...
      if ( (is_numeric($country) == false) || ($country < 1) ) {
        ...
        $messageStack->add('checkout_address', KUUZU::getDef('entry_country_error'));
      }
      ...
      if ($error == false) {
        ...
        $_SESSION['billto'] = $KUUZU_Db->lastInsertId();
...
// if no billing destination address was selected, use their own address as default
  ...
  $breadcrumb->add(KUUZU::getDef('navbar_title_1'), KUUZU::link('checkout_payment.php'));
  $breadcrumb->add(KUUZU::getDef('navbar_title_2'), KUUZU::link('checkout_payment_address.php'));
...
<?php echo HTML::form('checkout_address', KUUZU::link('checkout_payment_address.php'), 'post', 'class="form-horizontal"', ['tokenize' => true]); ?>
...
?>
  ...
  <h2><?php echo KUUZU::getDef('table_heading_payment_address'); ?></h2>

  <div class="contentText row">
    <div class="col-sm-8">
      <div class="alert alert-warning"><?php echo KUUZU::getDef('text_selected_payment_destination'); ?></div>
    </div>
    <div class="col-sm-4">
      <div class="panel panel-primary">
        <div class="panel-heading"><?php echo KUUZU::getDef('title_payment_address'); ?></div>
...
?>
  ...
  <h2><?php echo KUUZU::getDef('table_heading_address_book_entries'); ?></h2>

  <div class="alert alert-info"><?php echo KUUZU::getDef('text_select_other_payment_destination'); ?></div>
...
?>
  ...
  <h2><?php echo KUUZU::getDef('table_heading_new_payment_address'); ?></h2>

  <div class="alert alert-info"><?php echo KUUZU::getDef('text_create_new_payment_address'); ?></div>
...
?>
  ...
  <div class="buttonSet">
    <div class="text-right"><?php echo HTML::hiddenField('action', 'submit') . HTML::button(KUUZU::getDef('image_button_continue'), 'fa fa-angle-right', null, null, 'btn-success'); ?></div>
  </div>
    ...
    <div class="stepwizard">
      <div class="stepwizard-row">
        <div class="stepwizard-step">
          ...
          <p><?php echo KUUZU::getDef('checkout_bar_delivery'); ?></p>
        </div>
        <div class="stepwizard-step">
...
?>
  ...
  <div class="buttonSet">
    <?php echo HTML::button(KUUZU::getDef('image_button_back'), 'fa fa-angle-left', KUUZU::link('checkout_payment_address.php')); ?>
  </div>
