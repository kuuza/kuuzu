...
$KUUZU_Db = Db::initialize($_POST['DB_SERVER'], $_POST['DB_SERVER_USERNAME'], $_POST['DB_SERVER_PASSWORD'], $_POST['DB_DATABASE']);

Registry::set('Db', $KUUZU_Db);

$KUUZU_Db->setTablePrefix($_POST['DB_TABLE_PREFIX']);
...
$KUUZU_Db->save('configuration', ['configuration_value' => $_POST['CFG_STORE_NAME']], ['configuration_key' => 'STORE_NAME']);
$KUUZU_Db->save('configuration', ['configuration_value' => $_POST['CFG_STORE_OWNER_NAME']], ['configuration_key' => 'STORE_OWNER']);
$KUUZU_Db->save('configuration', ['configuration_value' => $_POST['CFG_STORE_OWNER_EMAIL_ADDRESS']], ['configuration_key' => 'STORE_OWNER_EMAIL_ADDRESS']);

if (!empty($_POST['CFG_STORE_OWNER_NAME']) && !empty($_POST['CFG_STORE_OWNER_EMAIL_ADDRESS'])) {
    $KUUZU_Db->save('configuration', ['configuration_value' => '"' . trim($_POST['CFG_STORE_OWNER_NAME']) . '" <' . trim($_POST['CFG_STORE_OWNER_EMAIL_ADDRESS']) . '>'], ['configuration_key' => 'EMAIL_FROM']);
} else {
    $KUUZU_Db->save('configuration', ['configuration_value' => $_POST['CFG_STORE_OWNER_EMAIL_ADDRESS']], ['configuration_key' => 'EMAIL_FROM']);
}

if (!empty($_POST['CFG_ADMINISTRATOR_USERNAME'])) {
    $Qcheck = $KUUZU_Db->prepare('select user_name from :table_administrators where user_name = :user_name');
    $Qcheck->bindValue(':user_name', $_POST['CFG_ADMINISTRATOR_USERNAME']);
    $Qcheck->execute();

    if ($Qcheck->fetch() !== false) {
        $KUUZU_Db->save('administrators', ['user_password' => Hash::encrypt(trim($_POST['CFG_ADMINISTRATOR_PASSWORD']))], ['user_name' => $_POST['CFG_ADMINISTRATOR_USERNAME']]);
    } else {
        $KUUZU_Db->save('administrators', ['user_name' => $_POST['CFG_ADMINISTRATOR_USERNAME'], 'user_password' => Hash::encrypt(trim($_POST['CFG_ADMINISTRATOR_PASSWORD']))]);
    }
}

if (FileSystem::isWritable(KUUZU::BASE_DIR . 'Work')) {
    if (!is_dir(Cache::getPath())) {
        mkdir(Cache::getPath(), 0777);
    }

    if (!is_dir(KUUZU::BASE_DIR . 'Work/Session')) {
        mkdir(KUUZU::BASE_DIR . 'Work/Session', 0777);
    }
}
...
file_put_contents(KUUZU::BASE_DIR . 'Conf/global.php', $file_contents);

@chmod(KUUZU::BASE_DIR . 'Conf/global.php', 0644);
...
file_put_contents(KUUZU::BASE_DIR . 'Sites/Shop/site_conf.php', $file_contents);

@chmod(KUUZU::BASE_DIR . 'Sites/Shop/site_conf.php', 0644);
...
file_put_contents(KUUZU::BASE_DIR . 'Sites/Admin/site_conf.php', $file_contents);

@chmod(KUUZU::BASE_DIR . 'Sites/Admin/site_conf.php', 0644);
        ...
        foreach ($m['modules'] as $module) {
            ...
            if (isset($module['params'])) {
                foreach ($module['params'] as $key => $value) {
                    $KUUZU_Db->save('configuration', ['configuration_value' => $value], ['configuration_key' => $key]);
                }
            }
        }

        $KUUZU_Db->save('configuration', ['configuration_value' => implode(';', $m_installed)], ['configuration_key' => $m['key']]);
    }
}
?>
...
<div class="row">
  <div class="col-sm-9">
    <div class="alert alert-info">
      ...
      <p>This web-based installation routine will correctly setup and configure Kuuzu to run on this server.</p>
      <p>Please follow the on-screen instructions that will take you through the database server, web server, and store configuration options. If help is needed at any stage, please consult the <a href="https://kuuzu.org/docs" target="_blank" class="alert-link">Kuuzu documentation</a>, seek help at the <a href="https://kuuzu.org/forum" target="_blank" class="alert-link">Kuuzu community forums</a>, visit the <a href="https://kuuzu.org/Support" target="_blank" class="alert-link">Kuuzu support page</a>, or send an enquiry to your server administrator or hosting server provider.</p>
    </div>
  </div>
  ...
  <div class="col-xs-12 col-sm-pull-9 col-sm-3">
    <div class="panel panel-success">
      ...
      <div class="panel-body">
        <p>Congratulations on installing and configuring Kuuzu as your online store solution!</p>
        ...
        <p>- <a href="https://kuuzu.org/Us&Team" target="_blank">The Kuuzu Team</a></p>
      </div>
    </div>
  </div>
</div>
