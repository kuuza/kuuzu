...
/**
  ...
  */
  ...
  $KUUZU_Language->loadDefinitions('modules/payment/sage_pay_server');
  ...
  if ( isset($_GET['skcode']) && isset($_POST['VPSSignature']) && isset($_POST['VPSTxId']) && isset($_POST['VendorTxCode']) && isset($_POST['Status']) ) {
    ...
    $Qsp = $KUUZU_Db->get('sagepay_server_securitykeys', 'securitykey', ['code' => $skcode], null, 1);
    ...
    if ($Qsp->fetch() !== false) {
      ...
      if ( $_POST['VPSSignature'] == $sig ) {
        if ( ($_POST['Status'] == 'OK') || ($_POST['Status'] == 'AUTHENTICATED') || ($_POST['Status'] == 'REGISTERED') ) {
          ...
          $KUUZU_Db->save('sagepay_server_securitykeys', ['verified' => 1, 'transaction_details' => $transaction_details_string], ['code' => $skcode]);

          $result = 'Status=OK' . chr(13) . chr(10) .
                    'RedirectURL=' . $sage_pay_server->formatURL(KUUZU::link('checkout_process.php', 'check=PROCESS&skcode=' . $skcode, false));
        } else {
          ...
          if ( MODULE_PAYMENT_SAGE_PAY_SERVER_PROFILE_PAGE == 'Normal' ) {
            $error_url = KUUZU::link('checkout_payment.php', 'payment_error=' . $sage_pay_server->code . (tep_not_null($error) ? '&error=' . $error : ''), false);
          } else {
            $error_url = KUUZU::link('ext/modules/payment/sage_pay/redirect.php', 'payment_error=' . $sage_pay_server->code . (tep_not_null($error) ? '&error=' . $error : ''), false);
          }
        ...
        $result = 'Status=INVALID' . chr(13) . chr(10) .
                  'RedirectURL=' . $sage_pay_server->formatURL(KUUZU::link('shopping_cart.php', '', false));
