  ...
  if (tep_not_null($action)) {
    switch ($action) {
      case 'save':
        ...
        KUUZU::redirect(FILENAME_MODULES, 'set=' . $set . '&module=' . $_GET['module']);
        break;
      case 'install':
      case 'remove':
        ...
        if (isset($module)) {
          if ($action == 'install') {
            ...
            KUUZU::redirect(FILENAME_MODULES, 'set=' . $set . '&module=' . $class);
            ...
            Registry::get('Db')->save('configuration', ['configuration_value' => implode(';', $modules_installed)], ['configuration_key' => $module_key]);
            KUUZU::redirect(FILENAME_MODULES, 'set=' . $set);
          }
        }
        KUUZU::redirect(FILENAME_MODULES, 'set=' . $set . '&module=' . $class);
        break;
...
<?php
  if (isset($_GET['list'])) {
    echo '            <td class="smallText" align="right">' . HTML::button(KUUZU::getDef('image_back'), 'fa fa-chevron-left', KUUZU::link(FILENAME_MODULES, 'set=' . $set)) . '</td>';
  } else {
    echo '            <td class="smallText" align="right">' . HTML::button(KUUZU::getDef('image_module_install') . ' (' . $new_modules_counter . ')', 'fa fa-plus', KUUZU::link(FILENAME_MODULES, 'set=' . $set . '&list=new')) . '</td>';
  }
?>
          </tr>
        </table></td>
          ...
          <tr>
            <td valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
              <tr class="dataTableHeadingRow">
                ...
                <td class="dataTableHeadingContent" align="right"><?php echo KUUZU::getDef('table_heading_sort_order'); ?></td>
...
<?php
  ...
  for ($i=0, $n=sizeof($directory_array); $i<$n; $i++) {
    ...
    if (strpos($file, '\\') !== false) {
      ...
      $KUUZU_Language->loadDefinitions($module_site . '/modules/' . $module_type . '/' . pathinfo($file, PATHINFO_FILENAME));
    ...
    if (isset($module)) {
      ...
      if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) {
        if ($module->check() > 0) {
          echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . KUUZU::link(FILENAME_MODULES, 'set=' . $set . '&module=' . $class . '&action=edit') . '\'">' . "\n";
        } else {
          echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">' . "\n";
        }
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . KUUZU::link(FILENAME_MODULES, 'set=' . $set . (isset($_GET['list']) ? '&list=new' : '') . '&module=' . $class) . '\'">' . "\n";
      }
?>
                ...
                <td class="dataTableContent" align="right"><?php if (isset($mInfo) && is_object($mInfo) && ($class == $mInfo->code) ) { echo HTML::image(KUUZU::linkImage('icon_arrow_right.gif')); } else { echo '<a href="' . KUUZU::link(FILENAME_MODULES, 'set=' . $set . (isset($_GET['list']) ? '&list=new' : '') . '&module=' . $class) . '">' . HTML::image(KUUZU::linkImage('icon_info.gif'), KUUZU::getDef('image_icon_info')) . '</a>'; } ?>&nbsp;</td>
              </tr>
<?php
    }
  }
  ...
  if (!isset($_GET['list'])) {
    ...
    $Qcheck = $KUUZU_Db->get('configuration', 'configuration_value', ['configuration_key' => $module_key]);
    ...
    if ($template_integration == true) {
      $Qcheck = $KUUZU_Db->get('configuration', 'configuration_value', ['configuration_key' => 'TEMPLATE_BLOCK_GROUPS']);
      if ($Qcheck->fetch() !== false) {
...
?>
              <tr>
                <td colspan="3" class="smallText"><?php echo KUUZU::getDef('text_module_directory') . ' ' . $module_directory; ?></td>
              </tr>
            </table></td>
<?php
  $heading = array();
  ...
  switch ($action) {
    case 'edit':
      ...
      $contents = array('form' => HTML::form('modules', KUUZU::link(FILENAME_MODULES, 'set=' . $set . '&module=' . $_GET['module'] . '&action=save')));
      $contents[] = array('text' => $keys);
      $contents[] = array('align' => 'center', 'text' => '<br />' . HTML::button(KUUZU::getDef('image_save'), 'fa fa-save') . HTML::button(KUUZU::getDef('image_cancel'), 'fa fa-close', KUUZU::link(FILENAME_MODULES, 'set=' . $set . '&module=' . $_GET['module'])));
      break;
    default:
      $heading[] = array('text' => '<strong>' . $mInfo->title . '</strong>');
      ...
      if (in_array($mInfo->code . $file_extension, $modules_installed) && ($mInfo->status > 0)) {
        ...
        $contents[] = array('align' => 'center', 'text' => HTML::button(KUUZU::getDef('image_edit'), 'fa fa-edit', KUUZU::link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=edit')) . HTML::button(KUUZU::getDef('image_module_remove'), 'fa fa-minus', KUUZU::link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=remove')));
        ...
        if (isset($mInfo)) {
          $contents[] = array('align' => 'center', 'text' => HTML::button(KUUZU::getDef('image_module_install'), 'fa fa-plus', KUUZU::link(FILENAME_MODULES, 'set=' . $set . '&module=' . $mInfo->code . '&action=install')));
