  ...
  */
  ...
  if ($VersionCache->exists(360)) {
    ...
    $releases = HTTP::getResponse([
      'url' => 'https://kuuzu.org/version/cart/' . $major_version . $minor_version
    ]);
  ...
  if (tep_not_null($action)) {
    switch ($action) {
      ...
      case 'getReleaseNotes':
        ...
        if ($ReleaseNotesCache->exists()) {
          ...
          $notes = HTTP::getResponse([
            'url' => 'https://kuuzu.org//version/cart/notes/' . $_POST['version'] . '.txt'
          ]);
      ...
      case 'downloadRelease':
        ...
        if (FileSystem::isWritable(KUUZU::BASE_DIR . 'Work/OnlineUpdates', true)) {
          if (!is_dir(KUUZU::BASE_DIR . 'Work/OnlineUpdates')) {
            mkdir(KUUZU::BASE_DIR . 'Work/OnlineUpdates', 0777, true);
          }

          $filepath = KUUZU::BASE_DIR . 'Work/OnlineUpdates/' . $_POST['version'] . '-update.zip';
          ...
          $downloadFile = HTTP::getResponse([
            'url' => 'https://kuuzu.org//?Products&Download=kuuzu-' . $_POST['version'] . '-ou',
            'method' => 'post'
          ]);
          ...
          $result['result'] = -2;
          $result['path'] = FileSystem::displayPath(KUUZU::BASE_DIR . 'Work/OnlineUpdates');
        }
      ...
      case 'applyRelease':
        ...
        try {
          if (!is_file(KUUZU::BASE_DIR . 'Work/Keys/kuuzu.pubkey')) {
            throw new \Exception('### ERROR ###' . "\n" . 'The following required public key cannot be found:' . "\n\n" . FileSystem::displayPath(KUUZU::BASE_DIR . 'Work/Keys/kuuzu.pubkey'));
          }

          if (!FileSystem::isWritable(KUUZU::BASE_DIR . 'version.txt')) {
            throw new \Exception('### ERROR ###' . "\n" . 'The following file cannot be written to - please check the file permissions: ' . "\n\n" . FileSystem::displayPath(KUUZU::BASE_DIR . 'version.txt'));
          }

          $update_zip = KUUZU::BASE_DIR . 'Work/OnlineUpdates/' . $_POST['version'] . '-update.zip';

          if (!is_file($update_zip)) {
            throw new \Exception('### ERROR ###' . "\n" . 'The following downloaded update package could not be found:' . "\n\n" . FileSystem::displayPath($update_zip));
          }

          $work_dir = KUUZU::BASE_DIR . 'Work/OnlineUpdates/update_contents';
          ...
          $public = openssl_get_publickey(file_get_contents(KUUZU::BASE_DIR . 'Work/Keys/kuuzu.pubkey'));
          ...
          if (!is_file($work_dir . '/' . $_POST['version'] . '/kuuzu.json')) {
            throw new \Exception('### ERROR ###' . "\n" . 'The kuuzu.json meta file could not be found in the following update package:' . "\n\n" . FileSystem::displayPath($update_pkg));
          }

          $meta = json_decode(file_get_contents($work_dir . '/' . $_POST['version'] . '/kuuzu.json'), true);

          if (!is_array($meta) || empty($meta)) {
            throw new \Exception('### ERROR ###' . "\n" . 'The kuuzu.json meta file in the following update package seems to be corrupt:' . "\n\n" . FileSystem::displayPath($update_pkg));
          }
          ...
          if (!isset($meta['version_req']) || ($meta['version_req'] != $current_version)) {
            throw new \Exception('### ERROR ###' . "\n" . 'The update package version does not match the required current version of Kuuzu. Current version: ' . $current_version . '; Update Package required version: ' . $meta['version_req']);
          }
          ...
          foreach ($update_pkg_contents as $file) {
            ...
            if (substr($pathname, 0, 8) == 'catalog/') {
              $file_source = KUUZU::getConfig('dir_root', 'Shop') . substr($pathname, 8);
            } elseif (substr($pathname, 0, 6) == 'admin/') {
              $file_source = KUUZU::getConfig('dir_root') . substr($pathname, 6);
            }
          ...
          if (is_file($work_dir . '/' . $_POST['version'] . '/delete.txt')) {
            ...
            foreach ($to_del as $d) {
              ...
              if (substr($d, 0, 8) == 'catalog/') {
                $file_source = KUUZU::getConfig('dir_root', 'Shop') . substr($d, 8);
              } elseif (substr($d, 0, 6) == 'admin/') {
                $file_source = KUUZU::getConfig('dir_root') . substr($d, 6);
              }
          ...
          if (is_file($work_dir . '/' . $_POST['version'] . '/Update.php')) {
            ...
            $OU = new Kuuzu\KU\OnlineUpdate\Update;
          ...
          if (isset($OU) && ($OU instanceof \Kuuzu\KU\OnlineUpdate\Update) && method_exists($OU, 'runBefore')) {
          ...
          if (isset($OU) && ($OU instanceof \Kuuzu\KU\OnlineUpdate\Update) && method_exists($OU, 'runAfter')) {
            OnlineUpdate::log('Executing update package runAfter()', $_POST['version']);

            $OU->runAfter();
          }

          if (file_put_contents(KUUZU::BASE_DIR . 'version.txt', $_POST['version'])) {
            OnlineUpdate::log('+ UPDATED: ' . FileSystem::displayPath(KUUZU::BASE_DIR . 'version.txt'), $_POST['version']);
          } else {
            OnlineUpdate::log('+++ UPDATE ERROR: Could not update the following file: ' . FileSystem::displayPath(KUUZU::BASE_DIR . 'version.txt'), $_POST['version']);
          }
  ...
  if (is_array($releases) && !empty($releases)) {
    ...
    if (!empty($new_version)) {
      $KUUZU_MessageStack->add(KUUZU::getDef('version_upgrades_available', ['version' => $new_version['version']]), 'warning', 'versionCheck');
    } else {
      $KUUZU_MessageStack->add(KUUZU::getDef('version_running_latest'), 'success', 'versionCheck');
    }
  } else {
    $KUUZU_MessageStack->add(KUUZU::getDef('error_could_not_connect'), 'error', 'versionCheck');
  }

  require($kuuTemplate->getFile('template_top.php'));
?>

<h2><i class="fa fa-cloud-download"></i> <a href="<?= KUUZU::link('online_update.php'); ?>"><?= KUUZU::getDef('heading_title'); ?></a></h2>

<div id="onlineUpdateContentBlock">
  <p><?php echo KUUZU::getDef('title_installed_version') . ' <strong>Kuuzu v' . $current_version . '</strong>'; ?></p>

  <?php echo $KUUZU_MessageStack->get('versionCheck'); ?>
...
<?php
    ...
    $contents[] = array('text' => 'Kuuzu has been successfully updated to the latest version!');
...
$(function() {
  ...
  var showVersionBlock = function(version) {
    ...
    $.post('<?= KUUZU::link('online_update.php', 'action=getReleaseNotes'); ?>', {version: version}, function(data) {
      ...
      $('#' + upDivId + ' .panel-body .row').prepend('<a href="https://kuuzu.org/?RPC&GotoReleaseAnnouncement&v=kuuzu-' + version + '-ou" class="pull-right btn btn-info btn-sm" target="_blank"><i class="fa fa-external-link fa-fw"></i> View Online</a>');
    }).fail(function() {
  ...
  $('#updateStartButton').on('click', function() {
    ...
    (function() {
      var runQueueInOrder = function(i) {
        ...
        $.post('<?= addslashes(KUUZU::link('online_update.php', 'action=downloadRelease')); ?>', {version: versions[i].version}, function(data) {
          if ((typeof data == 'object') && ('result' in data) && (data.result === 1)) {
            $('#' + upDivId + ' .panel-body .row').html('Applying..');

            $.post('<?= addslashes(KUUZU::link('online_update.php', 'action=applyRelease')); ?>', {version: versions[i].version}, function(data) {
              if ((typeof data == 'object') && ('result' in data) && (data.result === 1)) {
                ...
                $.post('<?= KUUZU::link('online_update.php', 'action=getUpdateLog'); ?>', {version: versions[i].version}, function(data) {
                  if ((typeof data == 'object') && ('result' in data) && (data.result === 1)) {
          ...
          $('#' + upDivId).removeClass('panel-primary').addClass('panel-danger');
          $('#' + upDivId + ' .panel-heading i[data-icon="status"]').removeClass('fa-spin').removeClass('fa-refresh').addClass('fa-exclamation-circle');
          $('#' + upDivId + ' .panel-body .row').html('Error!<br /><br />Could not connect to the Kuuzu Website to download the update package. Please try again.');
        });
      }
...
<script src="<?= KUUZU::link('Shop/ext/anchorme/anchorme.min.js'); ?>"></script>
