  ...
  */
  ...
  class d_total_customers {
    ...
    function d_total_customers() {
      $this->title = KUUZU::getDef('module_admin_dashboard_total_customers_title');
      $this->description = KUUZU::getDef('module_admin_dashboard_total_customers_description');
    ...
    function getOutput() {
      ...
      $Qorders = $KUUZU_Db->query('select date_format(customers_info_date_account_created, "%Y-%m-%d") as dateday, count(*) as total from :table_customers_info where date_sub(curdate(), interval 7 day) <= customers_info_date_account_created group by dateday');
      ...
      $chart_label = HTML::output(KUUZU::getDef('module_admin_dashboard_total_customers_chart_link'));
      $chart_label_link = KUUZU::link(FILENAME_CUSTOMERS);
