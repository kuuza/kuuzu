  ...
  */
  ...
  class d_admin_logins {
    ...
    function d_admin_logins() {
      $this->title = KUUZU::getDef('module_admin_dashboard_admin_logins_title');
      $this->description = KUUZU::getDef('module_admin_dashboard_admin_logins_description');
    ...
    function getOutput() {
      ...
      $output = '<table class="table table-hover">
                   <thead>
                     <tr class="info">
                       <th>' . KUUZU::getDef('module_admin_dashboard_admin_logins_title') . '</th>
                       <th class="text-right">' . KUUZU::getDef('module_admin_dashboard_admin_logins_date') . '</th>
                     </tr>
                   </thead>
                   <tbody>';

      $Qlogins = $KUUZU_Db->get('action_recorder', [
        'id',
        'user_name',
        'success',
        'date_added'
      ...
      while ($Qlogins->fetch()) {
        $output .= '    <tr>
                          <td><i class="fa fa-' . (($Qlogins->valueInt('success') === 1) ? 'check text-success' : 'times text-danger') . '"></i>&nbsp;<a href="' . KUUZU::link(FILENAME_ACTION_RECORDER, 'module=ar_admin_login&aID=' . $Qlogins->valueInt('id')) . '">' . $Qlogins->valueProtected('user_name') . '</a></td>
