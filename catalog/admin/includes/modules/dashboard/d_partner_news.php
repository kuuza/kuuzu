  ...
  */
  ...
  class d_partner_news {
    ...
    function d_partner_news() {
      $this->title = KUUZU::getDef('module_admin_dashboard_partner_news_title');
      $this->description = KUUZU::getDef('module_admin_dashboard_partner_news_description');
      ...
      if ( !function_exists('json_decode') ) {
        $this->description .= '<p style="color: #ff0000; font-weight: bold;">' . KUUZU::getDef('module_admin_dashboard_partner_news_error_json_decode') . '</p>';
    ...
    function getOutput() {
      ...
      if (is_array($result) && !empty($result)) {
        $output = '<table class="table table-hover">
                    <thead>
                      <tr class="info">
                        <th>' . KUUZU::getDef('module_admin_dashboard_partner_news_title') . '</th>
                      </tr>
                    </thead>
                    <tbody>';
        ...
        $output .= '    <tr>
                          <td class="text-right"><a href="https://kuuzu.org/Services" target="_blank">' . KUUZU::getDef('module_admin_dashboard_partner_news_more_title') . '</a></td>
                        </tr>
                      </tbody>
                    </table>';
      }
    ...
    function _getContent() {
      ...
      $NewsCache = new Cache('kuuzu_website-partner_news');

      if ($NewsCache->exists(60)) {
        $result = $NewsCache->get();
      } else {
        $response = HTTP::getResponse([
          'url' => 'https://kuuzu.org/index.php?RPC&Website&Index&GetPartnerStatusUpdates'
        ]);
    ...
    function install() {
      ...
      $KUUZU_Db->save('configuration', [
        ...
        'configuration_description' => 'Do you want to show the latest Kuuzu Partner News on the dashboard?',
