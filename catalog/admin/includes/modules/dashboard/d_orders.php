...
/**
  ...
  */
  ...
  class d_orders {
    ...
    function d_orders() {
      $this->title = KUUZU::getDef('module_admin_dashboard_orders_title');
      $this->description = KUUZU::getDef('module_admin_dashboard_orders_description');
    ...
    function getOutput() {
      ...
      $output = '<table class="table table-hover">
                   <thead>
                     <tr class="info">
                       <th>' . KUUZU::getDef('module_admin_dashboard_orders_title') . '</th>
                       <th>' . KUUZU::getDef('module_admin_dashboard_orders_total') . '</th>
                       <th>' . KUUZU::getDef('module_admin_dashboard_orders_date') . '</th>
                       <th>' . KUUZU::getDef('module_admin_dashboard_orders_order_status') . '</th>
                     </tr>
                   </thead>
                   <tbody>';

      $Qorders = $KUUZU_Db->get([
        'orders o',
        'orders_total ot',
        'orders_status s'
      ], [
        ...
        's.language_id' => $KUUZU_Language->getId()
      ], 'date_last_modified desc', 6);

      while ($Qorders->fetch()) {
        $output .= '    <tr>
                          <td><a href="' . KUUZU::link(FILENAME_ORDERS, 'oID=' . $Qorders->valueInt('orders_id') . '&action=edit') . '">' . $Qorders->valueProtected('customers_name') . '</a></td>
