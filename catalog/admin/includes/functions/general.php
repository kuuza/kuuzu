...
// Parse the data used in the html tags to ensure the tags will not break
  ...
  function tep_get_path($current_category_id = '') {
    ...
    if ($current_category_id == '') {
      ...
      if (sizeof($cPath_array) == 0) {
        ...
        $Qlast = $KUUZU_Db->get('categories', 'parent_id', ['categories_id' => (int)$cPath_array[(sizeof($cPath_array)-1)]]);

        $Qcurrent = $KUUZU_Db->get('categories', 'parent_id', ['categories_id' => (int)$current_category_id]);
  ...
  function tep_get_category_tree($parent_id = '0', $spacing = '', $exclude = '', $category_tree_array = '', $include_itself = false) {
    ...
    if ( (sizeof($category_tree_array) < 1) && ($exclude != '0') ) $category_tree_array[] = array('id' => '0', 'text' => KUUZU::getDef('text_top'));

    if ($include_itself) {
      $Qcategory = $KUUZU_Db->get('categories_description', 'categories_name', ['language_id' => $KUUZU_Language->getId(), 'categories_id' => (int)$parent_id]);
  ...
  function tep_options_name($options_id) {
    ...
    $Qoptions = $KUUZU_Db->get('products_options', 'products_options_name', ['products_options_id' => (int)$options_id, 'language_id' => $KUUZU_Language->getId()]);
  ...
  function tep_values_name($values_id) {
    ...
    $Qvalues = $KUUZU_Db->get('products_options_values', 'products_options_values_name', ['products_options_values_id' => (int)$values_id, 'language_id' => $KUUZU_Language->getId()]);

    return $Qvalues->value('products_options_values_name');
  }

  function tep_info_image($image, $alt, $width = '', $height = '') {
    if (tep_not_null($image) && is_file(KUUZU::getConfig('dir_root', 'Shop') . 'images/' . $image)) {
      $image = HTML::image(KUUZU::linkImage('Shop/' . $image), $alt, $width, $height);
    } else {
      $image = KUUZU::getDef('text_image_nonexistent');
    }
...
// Text Mode
  ...
  function tep_get_orders_status_name($orders_status_id, $language_id = '') {
    ...
    if (empty($language_id) || !is_numeric($language_id)) $language_id = $KUUZU_Language->getId();

    $Qstatus = $KUUZU_Db->get('orders_status', 'orders_status_name', ['orders_status_id' => (int)$orders_status_id, 'language_id' => (int)$language_id]);
...
// TABLES: products, products_to_categories, categories
  function tep_products_in_category_count($categories_id, $include_deactivated = false) {
    ...
    $Qchildren = $KUUZU_Db->get('categories', 'categories_id', ['parent_id' => (int)$categories_id]);
...
// return an array with country zones
  ...
  function tep_prepare_country_zones_pull_down($country_id = '') {
    ...
    if (sizeof($zones) > 0) {
      $zones_select = array(array('id' => '', 'text' => KUUZU::getDef('please_select')));
      $zones = array_merge($zones_select, $zones);
    } else {
      $zones = array(array('id' => '', 'text' => KUUZU::getDef('type_below')));
    }
...
// Retreive server information
  function tep_get_system_information() {
    ...
    $Qdate = $KUUZU_Db->query('select now() as datetime');

    @list($system, $host, $kernel) = preg_split('/[\s,]+/', @exec('uname -a'), 5);

    $data = array();

    $data['kuuzu']  = array('version' => KUUZU::getVersion());
    ...
    $data['mysql']  = array('version' => $KUUZU_Db->getAttribute(\PDO::ATTR_SERVER_VERSION),
  ...
  function tep_generate_category_path($id, $from = 'category', $categories_array = '', $index = 0) {
    ...
    if ($from == 'product') {
      $Qcategories = $KUUZU_Db->get('products_to_categories', 'categories_id', ['products_id' => (int)$id]);

      while ($Qcategories->fetch()) {
        if ($Qcategories->valueInt('categories_id') === 0) {
          $categories_array[$index][] = [
            'id' => '0',
            'text' => KUUZU::getDef('text_top')
          ];
        } else {
  ...
  function tep_output_generated_category_path($id, $from = 'category') {
    ...
    if (strlen($calculated_category_path_string) < 1) $calculated_category_path_string = KUUZU::getDef('text_top');
  ...
  function tep_remove_category($category_id) {
    ...
    $Qimage = $KUUZU_Db->get('categories', 'categories_image', ['categories_id' => (int)$category_id]);

    $Qduplicate = $KUUZU_Db->get('categories', 'categories_id', [
    ...
    if ($Qduplicate->fetch() === false) {
      if (!empty($Qimage->value('categories_image')) && is_file(KUUZU::getConfig('dir_root', 'Shop') . 'images/' . $Qimage->value('categories_image'))) {
        unlink(KUUZU::getConfig('dir_root', 'Shop') . 'images/' . $Qimage->value('categories_image'));
      }
    }

    $KUUZU_Db->delete('categories', ['categories_id' => (int)$category_id]);
    $KUUZU_Db->delete('categories_description', ['categories_id' => (int)$category_id]);
    $KUUZU_Db->delete('products_to_categories', ['categories_id' => (int)$category_id]);
  ...
  function tep_remove_product($product_id) {
    ...
    $Qimage = $KUUZU_Db->get('products', 'products_image', ['products_id' => (int)$product_id]);

    $Qduplicate = $KUUZU_Db->get('products', 'products_id', [
    ...
    if ($Qduplicate->fetch() === false) {
      if (!empty($Qimage->value('products_image')) && is_file(KUUZU::getConfig('dir_root', 'Shop') . 'images/' . $Qimage->value('products_image'))) {
        unlink(KUUZU::getConfig('dir_root', 'Shop') . 'images/' . $Qimage->value('products_image'));
      }
    }

    $Qimages = $KUUZU_Db->get('products_images', 'image', ['products_id' => (int)$product_id]);

    if ($Qimages->fetch() !== false) {
      do {
        $Qduplicate = $KUUZU_Db->get('products_images', 'id', [
      ...
      $KUUZU_Db->delete('products_images', ['products_id' => (int)$product_id]);
    }

    $KUUZU_Db->delete('specials', ['products_id' => (int)$product_id]);
    $KUUZU_Db->delete('products', ['products_id' => (int)$product_id]);
    $KUUZU_Db->delete('products_to_categories', ['products_id' => (int)$product_id]);
    $KUUZU_Db->delete('products_description', ['products_id' => (int)$product_id]);
    $KUUZU_Db->delete('products_attributes', ['products_id' => (int)$product_id]);

    $Qdel = $KUUZU_Db->prepare('delete from :table_customers_basket where products_id = :products_id or products_id like :products_id_att');
    $Qdel->bindInt(':products_id', (int)$product_id);
    $Qdel->bindInt(':products_id_att', (int)$product_id . '{%');
    $Qdel->execute();

    $Qdel = $KUUZU_Db->prepare('delete from :table_customers_basket_attributes where products_id = :products_id or products_id like :products_id_att');
    $Qdel->bindInt(':products_id', (int)$product_id);
    $Qdel->bindInt(':products_id_att', (int)$product_id . '{%');
    $Qdel->execute();

    $Qreviews = $KUUZU_Db->get('reviews', 'reviews_id', ['products_id' => (int)$product_id]);

    while ($Qreviews->fetch()) {
      $KUUZU_Db->delete('reviews_description', ['reviews_id' => $Qreviews->valueInt('reviews_id')]);
    }

    $KUUZU_Db->delete('reviews', ['products_id' => (int)$product_id]);
  ...
  function tep_remove_order($order_id, $restock = false) {
    ...
    if ($restock == 'on') {
      $Qproducts = $KUUZU_Db->get('orders_products', [
        'products_id',
        'products_quantity'
      ], [
      ...
      while ($Qproducts->fetch()) {
        $Qupdate = $KUUZU_Db->prepare('update :table_products set products_quantity = products_quantity + ' . $Qproducts->valueInt('products_quantity') . ', products_ordered = products_ordered - ' . $Qproducts->valueInt('products_quantity') . ' where products_id = :products_id');
        $Qupdate->bindInt(':products_id', $Qproducts->valueInt('products_id'));
        $Qupdate->execute();
      }
    }

    $KUUZU_Db->delete('orders', ['orders_id' => (int)$order_id]);
    $KUUZU_Db->delete('orders_products', ['orders_id' => (int)$order_id]);
    $KUUZU_Db->delete('orders_products_attributes', ['orders_id' => (int)$order_id]);
    $KUUZU_Db->delete('orders_status_history', ['orders_id' => (int)$order_id]);
    $KUUZU_Db->delete('orders_total', ['orders_id' => (int)$order_id]);
  }
...
// TABLES: tax_rates
  ...
  function tep_cfg_pull_down_order_statuses($order_status_id, $key = '') {
    ...
    $statuses_array = [
      [
        'id' => '0',
        'text' => KUUZU::getDef('text_default')
      ]
    ];
  ...
  function tep_get_order_status_name($order_status_id, $language_id = '') {
    ...
    if ($order_status_id < 1) return KUUZU::getDef('text_default');

    if (empty($language_id) || !is_numeric($language_id)) $language_id = $KUUZU_Language->getId();

    $Qstatus = $KUUZU_Db->get('orders_status', 'orders_status_name', ['orders_status_id' => (int)$order_status_id, 'language_id' => (int)$language_id]);
...
// TABLES: zones
  function tep_js_zone_list($country, $form, $field) {
    ...
    $Qcountries = $KUUZU_Db->get('zones', 'distinct zone_country_id', null, 'zone_country_id');
    ...
    while ($Qcountries->fetch()) {
      ...
      while ($Qstates->fetch()) {
        if ($num_state == '1') $output_string .= '    ' . $form . '.' . $field . '.options[0] = new Option("' . KUUZU::getDef('please_select') . '", "");' . "\n";
    ...
    $output_string .= '  } else {' . "\n" .
                      '    ' . $form . '.' . $field . '.options[0] = new Option("' . KUUZU::getDef('type_below') . '", "");' . "\n" .
                      '  }' . "\n";
