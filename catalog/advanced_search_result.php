...
/**
  ...
  */
  ...
  if ( (isset($_GET['keywords']) && empty($_GET['keywords'])) &&
       (isset($_GET['dfrom']) && (empty($_GET['dfrom']) || ($_GET['dfrom'] == KUUZU::getDef('dob_format_string')))) &&
       (isset($_GET['dto']) && (empty($_GET['dto']) || ($_GET['dto'] == KUUZU::getDef('dob_format_string')))) &&
       (isset($_GET['pfrom']) && !is_numeric($_GET['pfrom'])) &&
       (isset($_GET['pto']) && !is_numeric($_GET['pto'])) ) {
    $error = true;

    $messageStack->add_session('search', KUUZU::getDef('error_at_least_one_input'));
  } else {
    $dfrom = '';
    $dto = '';
    $pfrom = '';
    ...
    if (isset($_GET['dfrom'])) {
      $dfrom = (($_GET['dfrom'] == KUUZU::getDef('dob_format_string')) ? '' : $_GET['dfrom']);
    }

    if (isset($_GET['dto'])) {
      $dto = (($_GET['dto'] == KUUZU::getDef('dob_format_string')) ? '' : $_GET['dto']);
    }
    ...
    if (tep_not_null($dfrom)) {
      ...
      if ($dfromDateTime->isValid() === false) {
        ...
        $messageStack->add_session('search', KUUZU::getDef('error_invalid_from_date'));
      }
    }
    ...
    if (tep_not_null($dto)) {
      ...
      if ($dtoDateTime->isValid() === false) {
        ...
        $messageStack->add_session('search', KUUZU::getDef('error_invalid_to_date'));
      }
    }
    ...
    if (($date_check_error == false) && isset($dfromDateTime) && $dfromDateTime->isValid() && isset($dtoDateTime) && $dtoDateTime->isValid()) {
      if ($dfromDateTime->get() > $dtoDateTime->get()) {
        ...
        $messageStack->add_session('search', KUUZU::getDef('error_to_date_less_than_from_date'));
      }
    }
    ...
    if (tep_not_null($pfrom)) {
      if (!settype($pfrom, 'double')) {
        ...
        $messageStack->add_session('search', KUUZU::getDef('error_price_from_must_be_num'));
      }
    }
    ...
    if (tep_not_null($pto)) {
      if (!settype($pto, 'double')) {
        ...
        $messageStack->add_session('search', KUUZU::getDef('error_price_to_must_be_num'));
      }
    }
    ...
    if (($price_check_error == false) && is_float($pfrom) && is_float($pto)) {
      if ($pfrom >= $pto) {
        ...
        $messageStack->add_session('search', KUUZU::getDef('error_price_to_less_than_price_from'));
      }
    }
    ...
    if (tep_not_null($keywords)) {
      ...
      if (empty($search_keywords)) {
        ...
        $messageStack->add_session('search', KUUZU::getDef('error_invalid_keywords'));
      }
    }
  }
  ...
  if ($error == true) {
    KUUZU::redirect('advanced_search.php', tep_get_all_get_params(), true, false);
  }

  $breadcrumb->add(KUUZU::getDef('navbar_title_1'), KUUZU::link('advanced_search.php'));
  $breadcrumb->add(KUUZU::getDef('navbar_title_2'), KUUZU::link('advanced_search_result.php', tep_get_all_get_params(), true, false));

  require($kuuTemplate->getFile('template_top.php'));
?>

<div class="page-header">
  <h1><?php echo KUUZU::getDef('heading_title_2'); ?></h1>
</div>
...
?>
  ...
  <div class="buttonSet">
    <?php echo HTML::button(KUUZU::getDef('image_button_back'), 'fa fa-angle-left', KUUZU::link('advanced_search.php', tep_get_all_get_params(array('sort', 'page')), true, false)); ?>
  </div>
</div>
