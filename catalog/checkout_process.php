...
// avoid hack attempts during the checkout procedure by checking the internal cartID
  ...
  $KUUZU_Language->loadDefinitions('checkout_process');
...
// load the before_process function from the payment modules
  ...
  $KUUZU_Db->save('orders', $sql_data_array);
  ...
  for ($i=0, $n=sizeof($order_totals); $i<$n; $i++) {
    ...
    $KUUZU_Db->save('orders_total', $sql_data_array);
  }
...
// otherwise, we have to build the query dynamically with a loop
        ...
        $Qstock = $KUUZU_Db->prepare($stock_query_sql);
        ...
        $Qstock = $KUUZU_Db->prepare('select products_quantity from :table_products where products_id = :products_id');
        $Qstock->bindInt(':products_id', tep_get_prid($order->products[$i]['id']));
        $Qstock->execute();
      }
...
// do not decrement quantities if products_attributes_filename exists
        ...
        if ($stock_left != $Qstock->valueInt('products_quantity')) {
          $KUUZU_Db->save('products', ['products_quantity' => (int)$stock_left], ['products_id' => tep_get_prid($order->products[$i]['id'])]);
        }

        if ( ($stock_left < 1) && (STOCK_ALLOW_CHECKOUT == 'false') ) {
         $KUUZU_Db->save('products', ['products_status' => '0'], ['products_id' => tep_get_prid($order->products[$i]['id'])]);
        }
      }
    }

// Update products_ordered (for bestsellers list)
    $Qupdate = $KUUZU_Db->prepare('update :table_products set products_ordered = products_ordered + :products_ordered where products_id = :products_id');
    ...
    $KUUZU_Db->save('orders_products', $sql_data_array);
    $order_products_id = $KUUZU_Db->lastInsertId();
...
//------insert customer choosen option to order--------
    ...
    if (isset($order->products[$i]['attributes'])) {
      ...
      for ($j=0, $n2=sizeof($order->products[$i]['attributes']); $j<$n2; $j++) {
        ...
        $Qattributes = $KUUZU_Db->prepare($attributes_query);
        ...
        $Qattributes->bindInt(':language_id', $KUUZU_Language->getId());
        $Qattributes->execute();
        ...
        $KUUZU_Db->save('orders_products_attributes', $sql_data_array);
        ...
        if ((DOWNLOAD_ENABLED == 'true') && $Qattributes->hasValue('products_attributes_filename') && tep_not_null($Qattributes->value('products_attributes_filename'))) {
          ...
          $KUUZU_Db->save('orders_products_download', $sql_data_array);
        }
...
// lets start with the email confirmation
  $email_order = STORE_NAME . "\n" .
                 ...
                 KUUZU::getDef('email_text_order_number') . ' ' . $insert_id . "\n" .
                 KUUZU::getDef('email_text_invoice_url') . ' ' . KUUZU::link('account_history_info.php', 'order_id=' . $insert_id, false) . "\n" .
                 KUUZU::getDef('email_text_date_ordered') . ' ' . strftime(KUUZU::getDef('date_format_long')) . "\n\n";
  if ($order->info['comments']) {
    $email_order .= HTML::outputProtected($order->info['comments']) . "\n\n";
  }
  $email_order .= KUUZU::getDef('email_text_products') . "\n" .
                  KUUZU::getDef('email_separator') . "\n" .
                  $products_ordered .
                  KUUZU::getDef('email_separator') . "\n";
  ...
  if ($order->content_type != 'virtual') {
    $email_order .= "\n" . KUUZU::getDef('email_text_delivery_address') . "\n" .
                    KUUZU::getDef('email_separator') . "\n" .
                    tep_address_label($_SESSION['customer_id'], $_SESSION['sendto'], 0, '', "\n") . "\n";
  }

  $email_order .= "\n" . KUUZU::getDef('email_text_billing_address') . "\n" .
                  KUUZU::getDef('email_separator') . "\n" .
                  tep_address_label($_SESSION['customer_id'], $_SESSION['billto'], 0, '', "\n") . "\n\n";
  if (isset($KUUZU_PM)) {
    $email_order .= KUUZU::getDef('email_text_payment_method') . "\n" .
                    KUUZU::getDef('email_separator') . "\n";
    $email_order .= $order->info['payment_method'] . "\n\n";
    if (isset($KUUZU_PM->email_footer)) {
      $email_order .= $KUUZU_PM->email_footer . "\n\n";
    }
  }

  $orderEmail = new Mail($order->customer['email_address'], $order->customer['firstname'] . ' ' . $order->customer['lastname'], STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER, KUUZU::getDef('email_text_subject'));
  $orderEmail->setBody($email_order);
  $orderEmail->send();

// send emails to other people
  if (SEND_EXTRA_ORDER_EMAILS_TO != '') {
    $extraEmail = new Mail(SEND_EXTRA_ORDER_EMAILS_TO, null, STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER, KUUZU::getDef('email_text_subject'));
...
// unregister session variables used during checkout
  ...
  KUUZU::redirect('checkout_success.php');
