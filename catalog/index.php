  ...
  if (KUUZU::hasSitePage()) {
    if (KUUZU::isRPC() === false) {
        ...
        if (empty($page_file) || !is_file($page_file)) {
          $page_file = KUUZU::getConfig('dir_root') . 'includes/error_documents/404.php';
        }
...
// the following cPath references come from application_top.php
  ...
  if (isset($cPath) && tep_not_null($cPath)) {
    $Qcheck = $KUUZU_Db->prepare('select products_id from :table_products_to_categories where categories_id = :categories_id limit 1');
    ...
    if ($Qcheck->fetch() !== false) {
      ...
      $Qcheck = $KUUZU_Db->prepare('select categories_id from :table_categories where parent_id = :parent_id');
  ...
  $KUUZU_Language->loadDefinitions('index');

  require($kuuTemplate->getFile('template_top.php'));

  if ($category_depth == 'nested') {
    $Qcategory = $KUUZU_Db->prepare('select cd.categories_name, c.categories_image, cd.categories_description from :table_categories c, :table_categories_description cd where c.categories_id = :categories_id and c.categories_id = cd.categories_id and cd.language_id = :language_id');
    $Qcategory->bindInt(':categories_id', $current_category_id);
    $Qcategory->bindInt(':language_id', $KUUZU_Language->getId());
    $Qcategory->execute();
?>
...
<div class="contentContainer">
  <div class="row">
    <?php echo $kuuTemplate->getContent('index_nested'); ?>
  </div>
</div>
...
// We show them all
    ...
    $Qlisting = $KUUZU_Db->prepare($search_query);
    ...
    $catname = KUUZU::getDef('heading_title', ['store_name' => STORE_NAME]);
    if (isset($_GET['manufacturers_id']) && !empty($_GET['manufacturers_id'])) {
      $Qtitle = $KUUZU_Db->prepare('select m.manufacturers_image, m.manufacturers_name as catname, mi.manufacturers_description as catdesc from :table_manufacturers m, :table_manufacturers_info mi where m.manufacturers_id = :manufacturers_id and m.manufacturers_id = mi.manufacturers_id and mi.languages_id = :languages_id');
      $Qtitle->bindInt(':manufacturers_id', $_GET['manufacturers_id']);
      $Qtitle->bindInt(':languages_id', $KUUZU_Language->getId());
      $Qtitle->execute();

      $catname = $Qtitle->value('catname');
    } elseif ($current_category_id) {
      $Qtitle = $KUUZU_Db->prepare('select c.categories_image, cd.categories_name as catname, cd.categories_description as catdesc from :table_categories c, :table_categories_description cd where c.categories_id = :categories_id and c.categories_id = cd.categories_id and cd.language_id = :language_id');
      $Qtitle->bindInt(':categories_id', $current_category_id);
      $Qtitle->bindInt(':language_id', $KUUZU_Language->getId());
      $Qtitle->execute();
...
<div class="page-header">
  <h1><?php echo KUUZU::getDef('heading_title', ['store_name' => STORE_NAME]); ?></h1>
</div>
...
<div class="row">
  <?php echo $kuuTemplate->getContent('index'); ?>
</div>
