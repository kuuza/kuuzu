...
/**
  ...
  */
  ...
  $KUUZU_Language->loadDefinitions('product_info');

  $product_exists = true;

  $Qproduct = $KUUZU_Db->prepare('select p.products_id, pd.products_name, pd.products_description, p.products_model, p.products_quantity, p.products_image, pd.products_url, p.products_price, p.products_tax_class_id, p.products_date_added, p.products_date_available, p.manufacturers_id from :table_products p, :table_products_description pd where p.products_id = :products_id and p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = :language_id');
...
<div class="contentContainer">
  <div class="contentText">
    <div class="alert alert-warning"><?php echo KUUZU::getDef('text_product_not_found'); ?></div>
  </div>
...
<?php
    ...
    $Qupdate = $KUUZU_Db->prepare('update :table_products_description set products_viewed = products_viewed + 1 where products_id = :products_id and language_id = :language_id');
    ...
    $products_name = '<a href="' . KUUZU::link('product_info.php', 'products_id=' . $Qproduct->valueInt('products_id')) . '" itemprop="url"><span itemprop="name">' . $Qproduct->value('products_name') . '</span></a>';
...
<?php echo HTML::form('cart_quantity', KUUZU::link('product_info.php', tep_get_all_get_params(array('action')) . 'action=add_product'), 'post', 'class="form-horizontal" role="form"'); ?>
...
<?php
    if ( !empty($Qproduct->value('products_image')) ) {
      ...
      echo HTML::image(KUUZU::linkImage($Qproduct->value('products_image')), null, null, null, 'itemprop="image" style="display:none;"');

      $photoset_layout = (int)MODULE_HEADER_TAGS_PRODUCT_COLORBOX_LAYOUT;

      $Qpi = $KUUZU_Db->get('products_images', ['image', 'htmlcontent'], ['products_id' => $Qproduct->valueInt('products_id')], 'sort_order');
      $pi = $Qpi->fetchAll();
...
<?php
        ...
        foreach ($pi as $image) {
          ...
          echo HTML::image(KUUZU::linkImage($image['image']), '', '', '', 'id="piGalImg_' . $pi_counter . '"') . "\n";
        }
?>
...
?>
    ...
    <div class="piGal pull-right">
      <?php echo HTML::image(KUUZU::linkImage($Qproduct->value('products_image')), $Qproduct->value('products_name')); ?>
    </div>
...
<?php
      do {
        ...
        $Qpo = $KUUZU_Db->prepare('select pov.products_options_values_id, pov.products_options_values_name, pa.options_values_price, pa.price_prefix from :table_products_attributes pa, :table_products_options_values pov where pa.products_id = :products_id and pa.options_id = :options_id and pa.options_values_id = pov.products_options_values_id and pov.language_id = :language_id');
        $Qpo->bindInt(':products_id', $Qproduct->valueInt('products_id'));
        $Qpo->bindInt(':options_id', $Qpa->valueInt('products_options_id'));
        $Qpo->bindInt(':language_id', $KUUZU_Language->getId());
        $Qpo->execute();
...
?>
    ...
    <div class="alert alert-info"><?php echo KUUZU::getDef('text_date_available', ['products_date_available' => DateTime::toLong($Qproduct->value('products_date_available'))]); ?></div>
...
<?php
    ...
    $Qr = $KUUZU_Db->prepare('select count(*) as count, avg(reviews_rating) as avgrating from :table_reviews r, :table_reviews_description rd where r.products_id = :products_id and r.reviews_id = rd.reviews_id and rd.languages_id = :languages_id and reviews_status = 1');
    $Qr->bindInt(':products_id', $Qproduct->valueInt('products_id'));
    $Qr->bindInt(':languages_id', $KUUZU_Language->getId());
    $Qr->execute();
...
?>
  ...
  <div class="buttonSet row">
    <div class="col-xs-6"><?php echo HTML::button(KUUZU::getDef('image_button_reviews') . (($has_rating === true) ? ' (' . $Qr->value('count') . ')' : ''), 'fa fa-commenting', KUUZU::link('product_reviews.php', tep_get_all_get_params())); ?></div>
    <div class="col-xs-6 text-right"><?php echo HTML::hiddenField('products_id', $Qproduct->valueInt('products_id')) . HTML::button(KUUZU::getDef('image_button_in_cart'), 'fa fa-shopping-cart', null, null, 'btn-success btn-product-info btn-buy'); ?></div>
  </div>
...
<?php
    ...
    if ( $Qproduct->valueInt('manufacturers_id') > 0 ) {
      $Qm = $KUUZU_Db->get('manufacturers', 'manufacturers_name', ['manufacturers_id' => $Qproduct->valueInt('manufacturers_id')]);
