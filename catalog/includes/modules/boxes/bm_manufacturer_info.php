  ...
  */
  ...
  class bm_manufacturer_info {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_manufacturer_info_title');
      $this->description = KUUZU::getDef('module_boxes_manufacturer_info_description');
      ...
      if (isset($_GET['products_id'])) {
        ...
        if ($Qmanufacturer->fetch() !== false) {
          ...
          if (!empty($Qmanufacturer->value('manufacturers_image'))) {
            $manufacturer_info_string .= '<div>' . HTML::image(KUUZU::linkImage($Qmanufacturer->value('manufacturers_image')), $Qmanufacturer->value('manufacturers_name')) . '</div>';
          }

          if (!empty($Qmanufacturer->value('manufacturers_url'))) {
            $manufacturer_info_string .= '<div class="text-center"><a href="' . KUUZU::link('redirect.php', 'action=manufacturer&manufacturers_id=' . $Qmanufacturer->valueInt('manufacturers_id')) . '" target="_blank">' . KUUZU::getDef('module_boxes_manufacturer_info_box_homepage', ['manufacturers_name' => $Qmanufacturer->value('manufacturers_name')]) . '</a></div>';
          }
