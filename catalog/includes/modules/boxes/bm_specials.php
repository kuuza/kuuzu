  ...
  */
  ...
  class bm_specials {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_specials_title');
      $this->description = KUUZU::getDef('module_boxes_specials_description');
    ...
    function execute() {
      ...
      if (!isset($_GET['products_id'])) {
        $Qcheck = $KUUZU_Db->prepare('select p.products_id from :table_specials s, :table_products p, :table_products_description pd where s.status = 1 and s.products_id = p.products_id and p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = :language_id order by s.specials_date_added desc limit ' . (int)MAX_RANDOM_SELECT_SPECIALS);
