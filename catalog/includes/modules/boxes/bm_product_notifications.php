  ...
  */
  ...
  class bm_product_notifications {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_product_notifications_title');
      $this->description = KUUZU::getDef('module_boxes_product_notifications_description');
    ...
    function execute() {
      ...
      if (isset($_GET['products_id'])) {
        ...
        if ($notification_exists == true) {
          $notif_contents = '<a href="' . KUUZU::link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=notify_remove') . '"><span class="fa fa-remove"></span> ' . KUUZU::getDef('module_boxes_product_notifications_box_notify_remove', ['products_name' => tep_get_products_name($_GET['products_id'])]) .'</a>';
        } else {
          $notif_contents = '<a href="' . KUUZU::link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=notify') . '"><span class="fa fa-envelope"></span> ' . KUUZU::getDef('module_boxes_product_notifications_box_notify', ['products_name' => tep_get_products_name($_GET['products_id'])]) .'</a>';
        }
