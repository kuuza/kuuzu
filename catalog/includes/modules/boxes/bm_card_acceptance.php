  ...
  */
  ...
  class bm_card_acceptance {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_card_acceptance_title');
      $this->description = KUUZU::getDef('module_boxes_card_acceptance_description');
    ...
    function execute() {
      ...
      if ( (substr(basename($PHP_SELF), 0, 8) != 'checkout') && tep_not_null(MODULE_BOXES_CARD_ACCEPTANCE_LOGOS) ) {
        ...
        foreach ( explode(';', MODULE_BOXES_CARD_ACCEPTANCE_LOGOS) as $logo ) {
          $output .= HTML::image(KUUZU::linkImage('Shop/card_acceptance/' . basename($logo)), null, null, null, null, false);
        }
  ...
  function bm_card_acceptance_show_logos($text) {
    ...
    if ( !empty($text) ) {
      ...
      foreach (explode(';', $text) as $card) {
        $output .= '<li style="padding: 2px;">' . HTML::image(KUUZU::linkImage('Shop/card_acceptance/' . basename($card)), basename($card)) . '</li>';
      }
  ...
  function bm_card_acceptance_edit_logos($values, $key) {
    ...
    if ( $dir = @dir(KUUZU::getConfig('dir_root', 'Shop') . 'images/card_acceptance') ) {
      while ( $file = $dir->read() ) {
        if ( !is_dir(KUUZU::getConfig('dir_root', 'Shop') . 'images/card_acceptance/' . $file) ) {
    ...
    $output = '<h3>' . KUUZU::getDef('module_boxes_card_acceptance_shown_cards') . '</h3>' .
              '<ul id="ca_logos" style="list-style-type: none; margin: 0; padding: 5px; margin-bottom: 10px;">';

    foreach ($values_array as $file) {
      $output .= '<li style="padding: 2px;">' . HTML::image(KUUZU::linkImage('Shop/card_acceptance/' . $file), $file) . HTML::hiddenField('bm_card_acceptance_logos[]', $file) . '</li>';
    }

    $output .= '</ul>';

    $output .= '<h3>' . KUUZU::getDef('module_boxes_card_acceptance_new_cards') . '</h3><ul id="new_ca_logos" style="list-style-type: none; margin: 0; padding: 5px; margin-bottom: 10px;">';
    ...
    $drag_here_li = '<li id="caLogoEmpty" style="background-color: #fcf8e3; border: 1px #faedd0 solid; color: #a67d57; padding: 5px;">' . addslashes(KUUZU::getDef('module_boxes_card_acceptance_drag_here')) . '</li>';
