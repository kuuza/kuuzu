  ...
  */
  ...
  class bm_order_history {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_order_history_title');
      $this->description = KUUZU::getDef('module_boxes_order_history_description');
...
// retreive the last x products purchased
        $Qorders = $KUUZU_Db->prepare('select distinct op.products_id from :table_orders o, :table_orders_products op, :table_products p where o.customers_id = :customers_id and o.orders_id = op.orders_id and op.products_id = p.products_id and p.products_status = 1 group by op.products_id order by o.date_purchased desc limit :limit');
        ...
        if ($Qorders->fetch() !== false) {
          ...
          $Qproducts = $KUUZU_Db->prepare('select products_id, products_name from :table_products_description where products_id in (' . implode(', ', $product_ids) . ') and language_id = :language_id order by products_name');
          $Qproducts->bindInt(':language_id', $KUUZU_Language->getId());
          $Qproducts->execute();

          while ($Qproducts->fetch()) {
            $customer_orders_string .= '<li><span class="pull-right"><a href="' . KUUZU::link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=cust_order&pid=' . $Qproducts->valueInt('products_id')) . '"><span class="fa fa-shopping-cart"></span></a></span><a href="' . KUUZU::link('product_info.php', 'products_id=' . $Qproducts->valueInt('products_id')) . '">' . $Qproducts->value('products_name') . '</a></li>';
          }
