  ...
  */
  ...
  class bm_best_sellers {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_best_sellers_title');
      $this->description = KUUZU::getDef('module_boxes_best_sellers_description');
    ...
    function execute() {
      global $current_category_id, $kuuTemplate;
      ...
      if (!isset($_GET['products_id'])) {
        ...
        $Qbest = $KUUZU_Db->prepare($sql);
        $Qbest->bindInt(':language_id', $KUUZU_Language->getId());
        ...
        if (count($best) >= MIN_DISPLAY_BESTSELLERS) {
          ...
          foreach ($best as $b) {
            $bestsellers_list .= '<li><a href="' . KUUZU::link('product_info.php', 'products_id=' . $b['products_id']) . '">' . $position . '. <span itemprop="itemListElement">' . $b['products_name'] . '</span></a></li>';
