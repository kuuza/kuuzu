  ...
  */
  ...
  class bm_reviews {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_reviews_title');
      $this->description = KUUZU::getDef('module_boxes_reviews_description');
    ...
    function execute() {
      ...
      $Qcheck = $KUUZU_Db->prepare($sql_query);
...
// display random review box
          ...
          $reviews_box_contents = '<div class="text-center"><a href="' . KUUZU::link('product_reviews.php', 'products_id=' . $Qreview->valueInt('products_id')) . '">' . HTML::image(KUUZU::linkImage($Qreview->value('products_image')), $Qreview->value('products_name'), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</a></div><div><a href="' . KUUZU::link('product_reviews.php', 'products_id=' . $Qreview->valueInt('products_id')) . '">' . $rand_review_text . '</a>...</div><div class="text-center" title="' .  KUUZU::getDef('module_boxes_reviews_box_text_of_5_stars', ['reviews_rating' => $Qreview->valueInt('reviews_rating')]) . '">' . HTML::stars($Qreview->valueInt('reviews_rating')) . '</div>';
        }
      } elseif (isset($_GET['products_id'])) {
// display 'write a review' box
        $reviews_box_contents = '<span class="fa fa-thumbs-up"></span> <a href="' . KUUZU::link('product_reviews_write.php', 'products_id=' . (int)$_GET['products_id']) . '">' . KUUZU::getDef('module_boxes_reviews_box_write_review') .'</a>';
      } else {
// display 'no reviews' box
        $reviews_box_contents = '<p>' . KUUZU::getDef('module_boxes_reviews_box_no_reviews') . '</p>';
      }
