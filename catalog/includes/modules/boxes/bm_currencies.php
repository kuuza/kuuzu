  ...
  */
  ...
  class bm_currencies {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_boxes_currencies_title');
      $this->description = KUUZU::getDef('module_boxes_currencies_description');
    ...
    function execute() {
      global $PHP_SELF, $currencies, $kuuTemplate;
      ...
      if (substr(basename($PHP_SELF), 0, 8) != 'checkout') {
        if (isset($currencies) && is_object($currencies) && (count($currencies->currencies) > 1)) {
          ...
          $form_output = HTML::form('currencies', KUUZU::link($PHP_SELF, '', false), 'get', null, ['session_id' => true]) . HTML::selectField('currency', $currencies_array, $_SESSION['currency'], 'onchange="this.form.submit();"') . $hidden_get_variables . '</form>';
