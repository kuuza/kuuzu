  ...
  */
  ...
  class ar_reset_password {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_action_recorder_reset_password_title');
      $this->description = KUUZU::getDef('module_action_recorder_reset_password_description');
    ...
    function canPerform($user_id, $user_name) {
      ...
      $Qcheck = $KUUZU_Db->prepare('select id from :table_action_recorder where module = :module and user_name = :user_name and date_added >= date_sub(now(), interval :limit_minutes minute) and success = 1 limit :limit_attempts');
