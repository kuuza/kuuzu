  ...
  */
  ...
  class ht_canonical {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_canonical_title');
      $this->description = KUUZU::getDef('module_header_tags_canonical_description');
    ...
    function execute() {
      global $PHP_SELF, $cPath, $kuuTemplate, $category_depth;

      if (basename($PHP_SELF) == 'product_info.php') {
        $kuuTemplate->addBlock('<link rel="canonical" href="' . KUUZU::link('product_info.php', 'products_id=' . (int)$_GET['products_id'], false) . '" />' . "\n", $this->group);
      } elseif (basename($PHP_SELF) == 'index.php') {
        if (isset($cPath) && tep_not_null($cPath) && ($category_depth == 'products')) {
          $kuuTemplate->addBlock('<link rel="canonical" href="' . KUUZU::link('index.php', 'view=all&cPath=' . $cPath, false) . '" />' . "\n", $this->group);
        } elseif (isset($_GET['manufacturers_id']) && tep_not_null($_GET['manufacturers_id'])) {
          $kuuTemplate->addBlock('<link rel="canonical" href="' . KUUZU::link('index.php', 'view=all&manufacturers_id=' . (int)$_GET['manufacturers_id'], false) . '" />' . "\n", $this->group);
        }
      }
      else {
        $view_all_pages = array('products_new.php', 'specials.php');
        if (in_array(basename($PHP_SELF), $view_all_pages)) {
          $kuuTemplate->addBlock('<link rel="canonical" href="' . KUUZU::link($PHP_SELF, 'view=all', false) . '" />' . "\n", $this->group);
        }
      }
    }
