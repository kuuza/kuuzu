  ...
  */
  ...
  class ht_category_seo {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_category_seo_title');
      $this->description = KUUZU::getDef('module_header_tags_category_seo_description');
    ...
    function execute() {
      global $PHP_SELF, $kuuTemplate, $categories, $current_category_id;
      ...
      if ( (basename($PHP_SELF) == 'index.php') && ($current_category_id > 0) ){
        ...
        if (tep_not_null($meta['categories_seo_description'])) {
          $kuuTemplate->addBlock('<meta name="description" content="' . HTML::output($meta['categories_seo_description']) . '" />' . PHP_EOL, $this->group);
        }
        if ( (tep_not_null($meta['categories_seo_keywords'])) && (MODULE_HEADER_TAGS_CATEGORY_SEO_KEYWORDS_STATUS == 'True') ) {
          $kuuTemplate->addBlock('<meta name="keywords" content="' . HTML::output($meta['categories_seo_keywords']) . '" />' . PHP_EOL, $this->group);
        }
      }
    }
