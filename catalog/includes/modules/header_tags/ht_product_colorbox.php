  ...
  */
  ...
  class ht_product_colorbox {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_product_colorbox_title');
      $this->description = KUUZU::getDef('module_header_tags_product_colorbox_description');
    ...
    function execute() {
      ...
      if (tep_not_null(MODULE_HEADER_TAGS_PRODUCT_COLORBOX_PAGES)) {
        ...
        if (in_array(basename($PHP_SELF), $pages_array)) {
          $kuuTemplate->addBlock('<script src="ext/photoset-grid/jquery.photoset-grid.min.js"></script>' . "\n", $this->group);
          $kuuTemplate->addBlock('<link rel="stylesheet" href="ext/colorbox/colorbox.css" />' . "\n", 'header_tags');
          $kuuTemplate->addBlock('<script src="ext/colorbox/jquery.colorbox-min.js"></script>' . "\n", $this->group);
          $kuuTemplate->addBlock('<script>var ImgCount = $(".piGal").data("imgcount"); $(function() {$(\'.piGal\').css({\'visibility\': \'hidden\'});$(\'.piGal\').photosetGrid({layout: ""+ ImgCount +"",width: \'100%\',highresLinks: true,rel: \'pigallery\',onComplete: function() {$(\'.piGal\').css({\'visibility\': \'visible\'});$(\'.piGal a\').colorbox({maxHeight: \'90%\',maxWidth: \'90%\', rel: \'pigallery\'});$(\'.piGal img\').each(function() {var imgid = $(this).attr(\'id\') ? $(this).attr(\'id\').substring(9) : 0;if ( $(\'#piGalDiv_\' + imgid).length ) {$(this).parent().colorbox({ inline: true, href: "#piGalDiv_" + imgid });}});}});});</script>', $this->group);
        }
      }
    }
  ...
  function ht_product_colorbox_thumbnail_number() {
    return KUUZU::getDef('module_header_tags_product_colorbox_thumbnail_layout', ['product_colorbox_layout' => MODULE_HEADER_TAGS_PRODUCT_COLORBOX_LAYOUT, 'sum_product_colorbox_layout' => array_sum(str_split(MODULE_HEADER_TAGS_PRODUCT_COLORBOX_LAYOUT))]);
  }
