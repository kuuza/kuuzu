  ...
  */
  ...
  class ht_google_analytics {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_google_analytics_title');
      $this->description = KUUZU::getDef('module_header_tags_google_analytics_description');
  ...
  _gaq.push([\'_trackPageview\']);' . "\n";
        ...
        if ( (MODULE_HEADER_TAGS_GOOGLE_ANALYTICS_EC_TRACKING == 'True') && (basename($PHP_SELF) == 'checkout_success.php') && isset($_SESSION['customer_id']) ) {
          $Qorder = $KUUZU_Db->get('orders', ['orders_id', 'billing_city', 'billing_state', 'billing_country'], ['customers_id' => $_SESSION['customer_id']], 'date_purchased desc', 1);

          if ($Qorder->fetch() !== false) {
            $totals = array();

            $Qtotals = $KUUZU_Db->get('orders_total', ['value', 'class'], ['orders_id' => $Qorder->valueInt('orders_id')]);
    ...
    \'' . $Qorder->valueProtected('billing_country') . '\' // country
            ...
            $Qproducts = $KUUZU_Db->prepare('select op.products_id, pd.products_name, op.final_price, op.products_quantity from :table_orders_products op, :table_products_description pd, :table_languages l where op.orders_id = :orders_id and op.products_id = pd.products_id and pd.language_id = l.languages_id and l.code = :code');
            $Qproducts->bindInt(':orders_id', $Qorder->valueInt('orders_id'));
            $Qproducts->bindValue(':code', DEFAULT_LANGUAGE);
            $Qproducts->execute();

            while ($Qproducts->fetch()) {
              $Qcategory = $KUUZU_Db->prepare('select cd.categories_name from :table_categories_description cd, :table_products_to_categories p2c, :table_languages l where p2c.products_id = :products_id and p2c.categories_id = cd.categories_id and cd.language_id = l.languages_id and l.code = :code');
    ...
    var s = document.getElementsByTagName(\'script\')[0]; s.parentNode.insertBefore(ga, s);
        ...
        $kuuTemplate->addBlock($header, $this->group);
      }
    }
