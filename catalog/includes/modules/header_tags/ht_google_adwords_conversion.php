  ...
  */
  ...
  class ht_google_adwords_conversion {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_header_tags_google_adwords_conversion_title');
      $this->description = KUUZU::getDef('module_header_tags_google_adwords_conversion_description');
    ...
    function execute() {
      ...
      if ( ($PHP_SELF == 'checkout_success.php') && isset($_SESSION['customer_id']) ) {
        $Qorder = $KUUZU_Db->get('orders', ['orders_id', 'currency', 'currency_value'], ['customers_id' => $_SESSION['customer_id']], 'date_purchased desc', 1);

        if ($Qorder->fetch() !== false) {
          $Qsubtotal = $KUUZU_Db->get('orders_total', 'value', ['orders_id' => $Qorder->valueInt('orders_id'), 'class' => 'ot_subtotal']);
...
EOD;
          ...
          $kuuTemplate->addBlock($output, $this->group);
        }
      }
    }
