  ...
  */
  ...
  class ht_product_opengraph {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_product_opengraph_title');
      $this->description = KUUZU::getDef('module_header_tags_product_opengraph_description');
    ...
    function execute() {
      ...
      if (basename($PHP_SELF) == 'product_info.php') {
        $Qproduct = $KUUZU_Db->prepare('select
                                          p.products_id,
                                          pd.products_name,
        ...
        if ($Qproduct->fetch() !== false) {
          ...
          $Qimage = $KUUZU_Db->get('products_images', 'image', ['products_id' => $Qproduct->valueInt('products_id')], 'sort_order', 1);

          if ($Qimage->fetch() !== false) {
            $products_image = $Qimage->value('image');
          }

          $data['og:image'] = KUUZU::linkImage($products_image);
          ...
          $data['og:url'] = KUUZU::link('product_info.php', 'products_id=' . $Qproduct->valueInt('products_id'), false);

          $data['product:availability'] = ( $Qproduct->valueInt('products_quantity') > 0 ) ? KUUZU::getDef('module_header_tags_product_opengraph_text_in_stock') : KUUZU::getDef('module_header_tags_product_opengraph_text_out_of_stock');
          ...
          $kuuTemplate->addBlock($result, $this->group);
        }
      }
    }
