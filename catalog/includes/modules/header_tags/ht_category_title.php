  ...
  */
  ...
  class ht_category_title {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_category_title_title');
      $this->description = KUUZU::getDef('module_header_tags_category_title_description');
    ...
    function execute() {
      global $PHP_SELF, $kuuTemplate, $current_category_id;
      ...
      if (basename($PHP_SELF) == 'index.php') {
        if ($current_category_id > 0) {
          $Qcategory = $KUUZU_Db->get('categories_description', ['categories_name', 'categories_seo_title'], ['categories_id' => $current_category_id, 'language_id' => $KUUZU_Language->getId()]);
          ...
          if ($Qcategory->fetch() !== false) {
            if ( tep_not_null($Qcategory->value('categories_seo_title')) && (MODULE_HEADER_TAGS_CATEGORY_TITLE_SEO_TITLE_OVERRIDE == 'True') ) {
              $kuuTemplate->setTitle($Qcategory->value('categories_seo_title') . KUUZU::getDef('module_header_tags_category_seo_separator') . $kuuTemplate->getTitle());
            }
            else {
              $kuuTemplate->setTitle($Qcategory->value('categories_name') . KUUZU::getDef('module_header_tags_category_seo_separator') . $kuuTemplate->getTitle());
            }
          }
        }
      }
