  ...
  */
  ...
  class ht_twitter_product_card {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_twitter_product_card_title');
      $this->description = KUUZU::getDef('module_header_tags_twitter_product_card_description');
    ...
    function execute() {
      ...
      if ( ($PHP_SELF == 'product_info.php') && isset($_GET['products_id']) ) {
        $Qproduct = $KUUZU_Db->prepare('select p.products_id, pd.products_name, pd.products_description, p.products_image from :table_products p, :table_products_description pd where p.products_id = :products_id and p.products_status = 1 and p.products_id = pd.products_id and pd.language_id = :language_id');
        ...
        if ($Qproduct->fetch() !== false) {
          ...
          $data['image'] = KUUZU::linkImage($products_image);
