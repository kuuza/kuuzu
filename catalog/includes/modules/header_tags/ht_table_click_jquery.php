  ...
  */
  ...
  class ht_table_click_jquery {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_table_click_jquery_title');
      $this->description = KUUZU::getDef('module_header_tags_table_click_jquery_description');
    ...
    function execute() {
      ...
      if (tep_not_null(MODULE_HEADER_TAGS_TABLE_CLICK_JQUERY_PAGES)) {
        ...
        if (in_array(basename($PHP_SELF), $pages_array)) {
          $kuuTemplate->addBlock('<script>$(\'.table tr.table-selection\').click(function() { $(\'.table tr\').removeClass(\'success\').find(\'input\').prop(\'checked\', false); $(this).addClass(\'success\').find(\'input\').prop(\'checked\', true); });</script>' . "\n", $this->group);
        }
      }
    }
