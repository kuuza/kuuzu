...
class mc360 {
    ...
    function validate_cfg(){
        ...
        if (!$this->key_valid){
            ...
            if ($api->errorMessage!=''){
                ...
                $this->key_valid = true;
                $KUUZU_Db->save('configuration', ['configuration_value' => 'true'], ['configuration_key' => 'MODULE_HEADER_TAGS_MAILCHIMP_360_KEY_VALID']);

                if (empty($this->store_id)){
                    $this->store_id = md5(uniqid(rand(), true));
                    $KUUZU_Db->save('configuration', ['configuration_value' => $this->store_id], ['configuration_key' => 'MODULE_HEADER_TAGS_MAILCHIMP_360_STORE_ID']);
                }
            }
        }
    ...
    function process() {
        ...
        $Qorder = $KUUZU_Db->get('orders', 'orders_id', ['customers_id' => $_SESSION['customer_id']], 'date_purchased desc', 1);
        ...
        $Qproducts = $KUUZU_Db->get('orders_products', ['products_id', 'products_model', 'products_name', 'products_tax', 'products_quantity', 'final_price'], ['orders_id' => $Qorder->valueInt('orders_id')]);
        ...
        foreach($products_array as $product){
            ...
            $Qcat = $KUUZU_Db->get('products_to_categories', 'categories_id', ['products_id' => $product['id']], null, 1);
            ...
            //now recurse up the categories tree...
                $Qcat = $KUUZU_Db->prepare('select c.categories_id, c.parent_id, cd.categories_name from :table_categories c inner join :table_categories_description cd on c.categories_id = cd.categories_id where c.categories_id = :categories_id');
