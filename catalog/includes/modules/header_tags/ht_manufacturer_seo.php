  ...
  */
  ...
  class ht_manufacturer_seo {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_header_tags_manufacturers_seo_title');
      $this->description = KUUZU::getDef('module_header_tags_manufacturers_seo_description');
    ...
    function execute() {
      ...
      if (basename($PHP_SELF) == 'index.php') {
        if (isset($_GET['manufacturers_id']) && is_numeric($_GET['manufacturers_id'])) {
          $Qmeta = $KUUZU_Db->get('manufacturers_info', ['manufacturers_seo_description', 'manufacturers_seo_keywords'], ['manufacturers_id' => (int)$_GET['manufacturers_id'], 'languages_id' => $KUUZU_Language->getId()]);

          if ($Qmeta->fetch() !== false) {
            if (tep_not_null($Qmeta->value('manufacturers_seo_description'))) {
              $kuuTemplate->addBlock('<meta name="description" content="' . HTML::output($Qmeta->value('manufacturers_seo_description')) . '" />' . PHP_EOL, $this->group);
            }
            if ( tep_not_null($Qmeta->value('manufacturers_seo_keywords')) && (MODULE_HEADER_TAGS_MANUFACTURERS_SEO_KEYWORDS_STATUS == 'True') ) {
              $kuuTemplate->addBlock('<meta name="keywords" content="' . HTML::output($Qmeta->value('manufacturers_seo_keywords')) . '" />' . PHP_EOL, $this->group);
            }
          }
        }
      }
