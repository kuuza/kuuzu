...
/*
  ...
  It appears that the Kuuzu shipping system automatically rounds the
...
// class constructor
    function __construct() {
      $this->code = 'zones';
      $this->title = KUUZU::getDef('module_shipping_zones_text_title');
      $this->description = KUUZU::getDef('module_shipping_zones_text_description');
...
// class methods
    function quote($method = '') {
      ...
      if ($dest_zone == 0) {
        ...
        for ($i=0; $i<$size; $i+=2) {
          if ($shipping_weight <= $zones_table[$i]) {
            ...
            $shipping_method = KUUZU::getDef('module_shipping_zones_text_way') . ' ' . $dest_country . ' : ' . $shipping_weight . ' ' . KUUZU::getDef('module_shipping_zones_text_units');
            break;
          }
        }
        ...
        if ($shipping == -1) {
          $shipping_cost = 0;
          $shipping_method = KUUZU::getDef('module_shipping_zones_undefined_rate');
        } else {
      ...
      $this->quotes = array('id' => $this->code,
                            'module' => KUUZU::getDef('module_shipping_zones_text_title'),
      ...
      if ($error == true) $this->quotes['error'] = KUUZU::getDef('module_shipping_zones_invalid_zone');
