...
// class constructor
    function __construct() {
      ...
      $this->code = 'table';
      $this->title = KUUZU::getDef('module_shipping_table_text_title');
      $this->description = KUUZU::getDef('module_shipping_table_text_description');
      ...
      if ( ($this->enabled == true) && ((int)MODULE_SHIPPING_TABLE_ZONE > 0) ) {
        $check_flag = false;
        $Qcheck = $KUUZU_Db->get('zones_to_geo_zones', 'zone_id', ['geo_zone_id' => MODULE_SHIPPING_TABLE_ZONE, 'zone_country_id' => $order->delivery['country']['id']], 'zone_id');
        while ($Qcheck->fetch()) {
...
// class methods
    function quote($method = '') {
      ...
      $this->quotes = array('id' => $this->code,
                            'module' => KUUZU::getDef('module_shipping_table_text_title'),
                            'methods' => array(array('id' => $this->code,
                                                     'title' => KUUZU::getDef('module_shipping_table_text_way'),
        ...
        for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
          ...
          if (isset($order->products[$i]['attributes'])) {
            foreach ( $order->products[$i]['attributes'] as $option => $value ) {
              $Qcheck = $KUUZU_Db->prepare('select pa.products_id from :table_products_attributes pa, :table_products_attributes_download pad where pa.products_id = :products_id and pa.options_values_id = :options_values_id and pa.products_attributes_id = pad.products_attributes_id');
