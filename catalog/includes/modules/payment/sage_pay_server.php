  ...
  */
  ...
  class sage_pay_server {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_payment_sage_pay_server_text_title');
      $this->public_title = KUUZU::getDef('module_payment_sage_pay_server_text_public_title');
      $this->description = KUUZU::getDef('module_payment_sage_pay_server_text_description');
      ...
      if ( !function_exists('curl_init') ) {
        $this->description = '<div class="secWarning">' . KUUZU::getDef('module_payment_sage_pay_server_error_admin_curl') . '</div>' . $this->description;
      ...
      if ( $this->enabled === true ) {
        if ( !tep_not_null(MODULE_PAYMENT_SAGE_PAY_SERVER_VENDOR_LOGIN_NAME) ) {
          $this->description = '<div class="secWarning">' . KUUZU::getDef('module_payment_sage_pay_server_error_admin_configuration') . '</div>' . $this->description;
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_SAGE_PAY_SERVER_ZONE > 0) ) {
        $check_flag = false;
        $Qcheck = $KUUZU_Db->get('zones_to_geo_zones', 'zone_id', ['geo_zone_id' => MODULE_PAYMENT_SAGE_PAY_SERVER_ZONE, 'zone_country_id' => $order->billing['country']['id']], 'zone_id');
        while ($Qcheck->fetch()) {
    ...
    function before_process() {
      ...
      if (isset($_GET['check']) && ($_GET['check'] == 'PROCESS')) {
        if ( isset($_GET['skcode']) && isset($_SESSION['sagepay_server_skey_code']) && ($_GET['skcode'] == $_SESSION['sagepay_server_skey_code']) ) {
          ...
          $Qsp = $KUUZU_Db->get('sagepay_server_securitykeys', ['verified', 'transaction_details'], ['code' => $skcode], null, 1);

          if ($Qsp->fetch() !== false) {
            unset($_SESSION['sagepay_server_skey_code']);

            $KUUZU_Db->delete('sagepay_server_securitykeys', ['code' => $skcode]);
        ...
        $params = array('VPSProtocol' => $this->api_version,
                        ...
                        'NotificationURL' => $this->formatURL(KUUZU::link('ext/modules/payment/sage_pay/server.php', 'check=SERVER&skcode=' . $_SESSION['sagepay_server_skey_code'], false)),
        ...
        if ($return['Status'] == 'OK') {
          $Qsp = $KUUZU_Db->get('sagepay_server_securitykeys', ['id', 'securitykey'], ['code' => $_SESSION['sagepay_server_skey_code']], null, 1);

          if ($Qsp->fetch() !== false) {
            if ( $Qsp->value('securitykey') != $return['SecurityKey'] ) {
              $KUUZU_Db->save('sagepay_server_securitykeys', ['securitykey' => $return['SecurityKey'], 'date_added' => 'now()'], ['id' => $Qsp->valueInt('id')]);
            }
          } else {
            $KUUZU_Db->save('sagepay_server_securitykeys', [
...
// unregister session variables used during checkout
        ...
        KUUZU::redirect('ext/modules/payment/sage_pay/redirect.php');
      }
    }

    function get_error() {
      $message = KUUZU::getDef('module_payment_sage_pay_server_error_general');
...
// don't show an error message for user cancelled/aborted transactions
        ...
        $message = $this->getErrorMessage($error_number) . ' ' . KUUZU::getDef('module_payment_sage_pay_server_error_general');
      }

      $error = array('title' => KUUZU::getDef('module_payment_sage_pay_server_error_title'),
                     'error' => $message);
    ...
    function getParams() {
      ...
      $Qcheck = $KUUZU_Db->query('show tables like "sagepay_server_securitykeys"');
...
// format prices without currency formatting
    ...
    function getTestLinkInfo() {
      $dialog_title = KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_title');
      $dialog_button_close = KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_button_close');
      $dialog_success = KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_success');
      $dialog_failed = KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_failed');
      $dialog_error = KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_error');
      $dialog_connection_time = KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_time');
...
EOD;
      ...
      $info = '<p><img src="images/icons/locked.gif" border="0">&nbsp;<a href="javascript:openTestConnectionDialog();" style="text-decoration: underline; font-weight: bold;">' . KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_link_title') . '</a></p>' .
      ...
      $info .= '</p><div id="testConnectionDialogProgress"><p>' . KUUZU::getDef('module_payment_sage_pay_server_dialog_connection_general_text') . '</p><div id="tcdprogressbar"></div></div></div>' .
               $js;
