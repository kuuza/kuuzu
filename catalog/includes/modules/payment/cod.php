  ...
  */
  ...
  class cod {
    ...
    function __construct() {
      ...
      $this->code = 'cod';
      $this->title = KUUZU::getDef('module_payment_cod_text_title');
      $this->description = KUUZU::getDef('module_payment_cod_text_description');
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_COD_ZONE > 0) ) {
        $check_flag = false;
        $Qcheck = $KUUZU_Db->get('zones_to_geo_zones', 'zone_id', ['geo_zone_id' => MODULE_PAYMENT_COD_ZONE, 'zone_country_id' => $order->delivery['country']['id']], 'zone_id');
        while ($Qcheck->fetch()) {
