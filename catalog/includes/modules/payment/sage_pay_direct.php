  ...
  */
  ...
  class sage_pay_direct {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_payment_sage_pay_direct_text_title');
      $this->public_title = KUUZU::getDef('module_payment_sage_pay_direct_text_public_title');
      $this->description = KUUZU::getDef('module_payment_sage_pay_direct_text_description');
      ...
      if ( !function_exists('curl_init') ) {
        $this->description = '<div class="secWarning">' . KUUZU::getDef('module_payment_sage_pay_direct_error_admin_curl') . '</div>' . $this->description;
      ...
      if ( $this->enabled === true ) {
        if ( !tep_not_null(MODULE_PAYMENT_SAGE_PAY_DIRECT_VENDOR_LOGIN_NAME) ) {
          $this->description = '<div class="secWarning">' . KUUZU::getDef('module_payment_sage_pay_direct_error_admin_configuration') . '</div>' . $this->description;
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_SAGE_PAY_DIRECT_ZONE > 0) ) {
        $check_flag = false;
        $Qcheck = $KUUZU_Db->get('zones_to_geo_zones', 'zone_id', ['geo_zone_id' => MODULE_PAYMENT_SAGE_PAY_DIRECT_ZONE, 'zone_country_id' => $order->billing['country']['id']], 'zone_id');
        while ($Qcheck->fetch()) {
    ...
    function selection() {
      ...
      if ( (MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS == 'True') && !isset($_SESSION['payment']) ) {
        $Qtokens = $KUUZU_Db->get('customers_sagepay_tokens', '1', ['customers_id' => $_SESSION['customer_id']], null, 1);
    ...
    function pre_confirmation_check() {
      if ( $this->templateClassExists() ) {
        $GLOBALS['kuuTemplate']->addBlock($this->getSubmitCardDetailsJavascript(), 'header_tags');
      }
    }
    ...
    function confirmation() {
      ...
      if ( MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS == 'True' ) {
        ...
        if ($Qtokens->fetch() !== false) {
          ...
          do {
            ...
            if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
              $content .= '<tr class="moduleRowExtra" id="sagepay_card_cvc_' . $Qtokens->valueInt('id') . '">' .
                          ...
                          '  <td valign="top">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_cvc') . '&nbsp;' . HTML::inputField('cc_cvc_tokens_nh-dns[' . $Qtokens->valueInt('id') . ']', '', 'size="5" maxlength="4"') . '</td>' .
                          '</tr>';
            }
          } while ($Qtokens->fetch());
          ...
          $content .= '<tr class="moduleRow" id="sagepay_card_0">' .
                      ...
                      '  <td valign="top">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_new') . '</td>' .
                      '</tr>' .
                      '</table>';
        }
      }
      ...
      $content .= '<table id="sagepay_table_new_card" border="0" width="100%" cellspacing="0" cellpadding="2">' .
                  '<tr>' .
                  '  <td width="30%">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_type') . '</td>' .
                  '  <td>' . HTML::selectField('cc_type', $card_types, '', 'id="sagepay_card_type"') . '</td>' .
                  '</tr>' .
                  '<tr>' .
                  '  <td width="30%">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_owner') . '</td>' .
                  '  <td>' . HTML::inputField('cc_owner', $order->billing['firstname'] . ' ' . $order->billing['lastname'], 'maxlength="50"') . '</td>' .
                  '</tr>' .
                  '<tr>' .
                  '  <td width="30%">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_number') . '</td>' .
      ...
      if ( (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True') || (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_AMEX == 'True') ) {
        $content .= '<tr>' .
                    '  <td width="30%">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_starts') . '</td>' .
                    '  <td>' . HTML::selectField('cc_starts_month', $months_array, '', 'id="sagepay_card_date_start"') . '&nbsp;' . HTML::selectField('cc_starts_year', $year_valid_from_array) . '&nbsp;' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_starts_info') . '</td>' .
                    '</tr>';
      }

      $content .= '<tr>' .
                  '  <td width="30%">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_expires') . '</td>' .
      ...
      if ( (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True') ) {
        $content .= '<tr>' .
                    '  <td width="30%">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_issue_number') . '</td>' .
                    '  <td>' . HTML::inputField('cc_issue_nh-dns', '', 'id="sagepay_card_issue" size="3" maxlength="2"') . '&nbsp;' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_issue_number_info') . '</td>' .
                    '</tr>';
      }

      if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
        $content .= '<tr>' .
                    '  <td width="30%">' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_cvc') . '</td>' .
      ...
      if ( MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS == 'True' ) {
        $content .= '<tr>' .
                    '  <td width="30%">&nbsp;</td>' .
                    '  <td>' . HTML::checkboxField('cc_save', 'true') . ' ' . KUUZU::getDef('module_payment_sage_pay_direct_credit_card_save') . '</td>' .
                    '</tr>';
      }
    ...
    function before_process() {
      ...
      if ( isset($_GET['check']) ) {
        if ( ($_GET['check'] == '3D') && isset($_POST['MD']) && tep_not_null($_POST['MD']) && isset($_POST['PaRes']) && tep_not_null($_POST['PaRes']) ) {
          ...
          if ( ($_POST['Status'] == 'PAYPALOK') && isset($_POST['VPSTxId']) && isset($_POST['CustomerEMail']) && isset($_POST['PayerID']) ) {
            ...
            KUUZU::redirect('checkout_confirmation.php');
          }
        }
      } else {
        $sagepay_token = null;
        ...
        if ( MODULE_PAYMENT_SAGE_PAY_DIRECT_TOKENS == 'True' ) {
          if ( isset($_POST['sagepay_card']) && is_numeric($_POST['sagepay_card']) && ($_POST['sagepay_card'] > 0) ) {
            $Qtoken = $KUUZU_Db->get('customers_sagepay_tokens', 'sagepay_token', ['id' => $_POST['sagepay_card'], 'customers_id' => $_SESSION['customer_id']]);
        ...
        if ( !isset($sagepay_token) ) {
          ...
          if ( !isset($cc_type) || ($this->isCard($cc_type) == false) ) {
            KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardtype');
          }
          ...
          if ( $cc_type != 'PAYPAL' ) {
            ...
            if ( !isset($cc_owner) || empty($cc_owner) ) {
              KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardowner');
            }

            if ( !isset($cc_number) || (is_numeric($cc_number) == false) ) {
              KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardnumber');
            }
            ...
            if ( (($cc_type == 'MAESTRO') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True')) || (($cc_type == 'AMEX') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_AMEX == 'True')) ) {
              if ( !isset($_POST['cc_starts_month']) || !in_array($_POST['cc_starts_month'], $months_array) ) {
                KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardstart');
              }
            ...
            if ( !isset($_POST['cc_expires_month']) || !in_array($_POST['cc_expires_month'], $months_array) ) {
              KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardexpires');
            }
            ...
            if ( (($cc_type == 'MAESTRO') && (MODULE_PAYMENT_SAGE_PAY_DIRECT_ALLOW_MAESTRO == 'True')) ) {
              if ( !isset($cc_issue) || empty($cc_issue) ) {
                KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardissue');
              }
            }

            if (MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_WITH_CVC == 'True') {
              if ( !isset($cc_cvc) || empty($cc_cvc) ) {
                KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . '&error=cardcvc');
              }
            }
          }
        }
        ...
        if ( isset($sagepay_token) ) {
          ...
          if ( $cc_type == 'PAYPAL' ) {
            $params['PayPalCallbackURL'] = KUUZU::link('checkout_process.php', 'check=PAYPAL');
          } else {
      ...
      if ($sage_pay_response['Status'] == '3DAUTH') {
        ...
        KUUZU::redirect('ext/modules/payment/sage_pay/checkout.php');
      }
      ...
      if ( ($sage_pay_response['Status'] != 'OK') && ($sage_pay_response['Status'] != 'AUTHENTICATED') && ($sage_pay_response['Status'] != 'REGISTERED') ) {
        ...
        KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code . (tep_not_null($error) ? '&error=' . $error : ''));
      }
    }
    ...
    function after_process() {
      ...
      if ( isset($sage_pay_response['Token']) && isset($_SESSION['sagepay_token_cc_number']) ) {
        $Qcheck = $KUUZU_Db->get('customers_sagepay_tokens', 'id', ['customers_id' => $_SESSION['customer_id'], 'sagepay_token' => $sage_pay_response['Token']], null, 1);
        ...
        if ($Qcheck->fetch() === false) {
          ...
          $KUUZU_Db->save('customers_sagepay_tokens', $sql_data_array);
        }
      ...
      $KUUZU_Db->save('orders_status_history', $sql_data_array);
    ...
    function get_error() {
      $message = KUUZU::getDef('module_payment_sage_pay_direct_error_general');

      if ( isset($_GET['error']) && tep_not_null($_GET['error']) ) {
        if ( is_numeric($_GET['error']) && $this->errorMessageNumberExists($_GET['error']) ) {
          $message = $this->getErrorMessage($_GET['error']) . ' ' . KUUZU::getDef('module_payment_sage_pay_direct_error_general');
        } else {
          switch ($_GET['error']) {
            case 'cardtype':
              $message = KUUZU::getDef('module_payment_sage_pay_direct_error_cardtype');
              break;

            case 'cardowner':
              $message = KUUZU::getDef('module_payment_sage_pay_direct_error_cardowner');
              break;

            case 'cardnumber':
              $message = KUUZU::getDef('module_payment_sage_pay_direct_error_cardnumber');
              break;

            case 'cardstart':
              $message = KUUZU::getDef('module_payment_sage_pay_direct_error_cardstart');
              break;

            case 'cardexpires':
              $message = KUUZU::getDef('module_payment_sage_pay_direct_error_cardexpires');
              break;

            case 'cardissue':
              $message = KUUZU::getDef('module_payment_sage_pay_direct_error_cardissue');
              break;

            case 'cardcvc':
              $message = KUUZU::getDef('module_payment_sage_pay_direct_error_cardcvc');
              break;
          }
        }
      }

      $error = array('title' => KUUZU::getDef('module_payment_sage_pay_direct_error_title'),
                     'error' => $message);
    ...
    function install($parameter = null) {
      ...
      foreach ($params as $key => $data) {
        ...
        $KUUZU_Db->save('configuration', $sql_data_array);
      }
    }
    ...
    function getParams() {
      ...
      $Qcheck = $KUUZU_Db->query('show tables like "customers_sagepay_tokens"');
...
EOD;
      ...
      if (!defined('MODULE_PAYMENT_SAGE_PAY_DIRECT_TRANSACTION_ORDER_STATUS_ID')) {
        $Qcheck = $KUUZU_Db->get('orders_status', 'orders_status_id', ['orders_status_name' => 'Sage Pay [Transactions]'], null, 1);

        if ($Qcheck->fetch() === false) {
          $Qstatus = $KUUZU_Db->get('orders_status', 'max(orders_status_id) as status_id');
    ...
    function sendTransactionToGateway($url, $parameters) {
      ...
      if ( MODULE_PAYMENT_SAGE_PAY_DIRECT_VERIFY_SSL == 'True' ) {
        ...
        if ( is_file(KUUZU::getConfig('dir_root', 'Shop') . 'ext/modules/payment/sage_pay/sagepay.com.crt') ) {
          curl_setopt($curl, CURLOPT_CAINFO, KUUZU::getConfig('dir_root', 'Shop') . 'ext/modules/payment/sage_pay/sagepay.com.crt');
        } elseif ( is_file(KUUZU::getConfig('dir_root', 'Shop') . 'includes/cacert.pem') ) {
          curl_setopt($curl, CURLOPT_CAINFO, KUUZU::getConfig('dir_root', 'Shop') . 'includes/cacert.pem');
        }
      } else {
...
// format prices without currency formatting
    ...
    function deleteCard($token, $token_id) {
      ...
      return $KUUZU_Db->delete('customers_sagepay_tokens', ['id' => $token_id, 'customers_id' => $_SESSION['customer_id'], 'sagepay_token' => $token]) === 1;
    }
    ...
    function getTestLinkInfo() {
      $dialog_title = KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_title');
      $dialog_button_close = KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_button_close');
      $dialog_success = KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_success');
      $dialog_failed = KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_failed');
      $dialog_error = KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_error');
      $dialog_connection_time = KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_time');

      $test_url = KUUZU::link('modules.php', 'set=payment&module=' . $this->code . '&action=install&subaction=conntest');
...
EOD;
      ...
      $info = '<p><img src="images/icons/locked.gif" border="0">&nbsp;<a href="javascript:openTestConnectionDialog();" style="text-decoration: underline; font-weight: bold;">' . KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_link_title') . '</a></p>' .
      ...
      $info .= '</p><div id="testConnectionDialogProgress"><p>' . KUUZU::getDef('module_payment_sage_pay_direct_dialog_connection_general_text') . '</p><div id="tcdprogressbar"></div></div></div>' .
               $js;
    ...
    function templateClassExists() {
      return class_exists('kuuTemplate') && isset($GLOBALS['kuuTemplate']) && is_object($GLOBALS['kuuTemplate']) && (get_class($GLOBALS['kuuTemplate']) == 'kuuTemplate');
    }
