  ...
  */
  ...
  class sage_pay_form {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_payment_sage_pay_form_text_title');
      $this->public_title = KUUZU::getDef('module_payment_sage_pay_form_text_public_title');
      $this->description = KUUZU::getDef('module_payment_sage_pay_form_text_description');
      ...
      if ( !function_exists('mcrypt_encrypt') ) {
        $this->description = '<div class="secWarning">' . KUUZU::getDef('module_payment_sage_pay_form_error_admin_mcrypt') . '</div>' . $this->description;
      ...
      if ( $this->enabled === true ) {
        if ( !tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_VENDOR_LOGIN_NAME) || !tep_not_null(MODULE_PAYMENT_SAGE_PAY_FORM_ENCRYPTION_PASSWORD) ) {
          $this->description = '<div class="secWarning">' . KUUZU::getDef('module_payment_sage_pay_form_error_admin_configuration') . '</div>' . $this->description;
    ...
    function update_status() {
      ...
      if ( ($this->enabled == true) && ((int)MODULE_PAYMENT_SAGE_PAY_FORM_ZONE > 0) ) {
        $check_flag = false;
        $Qcheck = $KUUZU_Db->get('zones_to_geo_zones', 'zone_id', ['geo_zone_id' => MODULE_PAYMENT_SAGE_PAY_FORM_ZONE, 'zone_country_id' => $order->billing['country']['id']], 'zone_id');
        while ($Qcheck->fetch()) {
    ...
    function process_button() {
      ...
      $crypt = array('ReferrerID' => 'C74D7B82-E9EB-4FBD-93DB-76F0F551C802',
                     ...
                     'SuccessURL' => KUUZU::link('checkout_process.php'),
                     'FailureURL' => KUUZU::link('checkout_payment.php', 'payment_error=' . $this->code),
    ...
    function before_process() {
      ...
      if (isset($_GET['crypt']) && tep_not_null($_GET['crypt'])) {
        ...
        KUUZU::redirect('checkout_payment.php', 'payment_error=' . $this->code);
      }
    }
    ...
    function get_error() {
      $message = KUUZU::getDef('module_payment_sage_pay_form_error_general');
...
// don't show an error message for user cancelled/aborted transactions
        ...
        $message = $this->getErrorMessage($error_number) . ' ' . KUUZU::getDef('module_payment_sage_pay_form_error_general');
      }

      $error = array('title' => KUUZU::getDef('module_payment_sage_pay_form_error_title'),
                     'error' => $message);
