  ...
  */
  ...
  class cm_in_category_listing {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_content_in_category_listing_title');
      $this->description = KUUZU::getDef('module_content_in_category_listing_description');
    ...
    function execute() {
      global $kuuTemplate, $category, $cPath_array, $cPath, $current_category_id;
...
// check to see if there are deeper categories within the current category
        ...
        for($i=0, $n=sizeof($category_links); $i<$n; $i++) {
          $Qcategories = $KUUZU_Db->prepare('select categories_id from :table_categories c, :table_categories_description cd where c.parent_id = :parent_id and c.categories_id = cd.categories_id and cd.language_id = :language_id limit 1');
          ...
          if (count($Qcategories->fetchAll()) < 1) {
            ...
            $Qcategories = $KUUZU_Db->prepare('select c.categories_id, cd.categories_name, c.categories_image, c.parent_id from :table_categories c, :table_categories_description cd where c.parent_id = :parent_id and c.categories_id = cd.categories_id and cd.language_id = :language_id order by sort_order, cd.categories_name');
            $Qcategories->bindInt(':parent_id', $category_links[$i]);
            $Qcategories->bindInt(':language_id', $KUUZU_Language->getId());
            $Qcategories->execute();
