  ...
  */
  ...
  class cm_cs_redirect_old_order {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_content_checkout_success_redirect_old_order_title');
      $this->description = KUUZU::getDef('module_content_checkout_success_redirect_old_order_description');
    ...
    function execute() {
      ...
      if ( (int)MODULE_CONTENT_CHECKOUT_SUCCESS_REDIRECT_OLD_ORDER_MINUTES > 0 ) {
        $Qcheck = $KUUZU_Db->prepare('select 1 from :table_orders where orders_id = :orders_id and date_purchased < date_sub(now(), interval :limit_minutes minute) limit 1');
