  ...
  */
  ...
  class cm_login_form {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_content_login_form_title');
      $this->description = KUUZU::getDef('module_content_login_form_description');
    ...
    function execute() {
      global $login_customer_id, $messageStack, $kuuTemplate;
...
// Check if email exists
        $Qcustomer = $KUUZU_Db->get('customers', ['customers_id', 'customers_password'], ['customers_email_address' => $email_address], null, 1);
...
// migrate old hashed password to new php password_hash
            if (Hash::needsRehash($Qcustomer->value('customers_password'))) {
              $KUUZU_Db->save('customers', ['customers_password' => Hash::encrypt($password)], ['customers_id' => $login_customer_id]);
            }
          }
        }
      }
      ...
      if ($error == true) {
        $messageStack->add('login', KUUZU::getDef('module_content_login_text_login_error'));
      }
