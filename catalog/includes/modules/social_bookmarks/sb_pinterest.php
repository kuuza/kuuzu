  ...
  */
  ...
  class sb_pinterest {
    ...
    function __construct() {
      $this->title = KUUZU::getDef('module_social_bookmarks_pinterest_title');
      $this->public_title = KUUZU::getDef('module_social_bookmarks_pinterest_public_title');
      $this->description = KUUZU::getDef('module_social_bookmarks_pinterest_description');
...
// add the js in the footer
      $kuuTemplate->addBlock('<script src="//assets.pinterest.com/js/pinit.js"></script>', 'footer_scripts');
...
// and image (used for media)
      $Qimage = $KUUZU_Db->get('products', 'products_image', ['products_id' => (int)$_GET['products_id']]);

      if (!empty($Qimage->value('products_image'))) {
        $image_file = $Qimage->value('products_image');

        $Qimage = $KUUZU_Db->get('products_images', 'image', ['products_id' => (int)$_GET['products_id']], 'sort_order');
        ...
        $params['media'] = KUUZU::linkImage($image_file);
      }

// url
      $params['url'] = KUUZU::link('product_info.php', 'products_id=' . $_GET['products_id'], false);
