  ...
  */
  ...
  class ot_shipping {
    ...
    function __construct() {
      ...
      $this->title = KUUZU::getDef('module_order_total_shipping_title');
      $this->description = KUUZU::getDef('module_order_total_shipping_description');
    ...
    function process() {
      ...
      if (MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING == 'true') {
        ...
        if ( ($pass == true) && ( ($order->info['total'] - $order->info['shipping_cost']) >= MODULE_ORDER_TOTAL_SHIPPING_FREE_SHIPPING_OVER) ) {
          $order->info['shipping_method'] = KUUZU::getDef('free_shipping_title');
      ...
      if (tep_not_null($order->info['shipping_method'])) {
        if ($KUUZU_SM->tax_class > 0) {
          $shipping_tax = tep_get_tax_rate($KUUZU_SM->tax_class, $order->delivery['country']['id'], $order->delivery['zone_id']);
          $shipping_tax_description = tep_get_tax_description($KUUZU_SM->tax_class, $order->delivery['country']['id'], $order->delivery['zone_id']);
