...
// Get last order id for checkout_success
    $Qorders = $KUUZU_Db->get('orders', 'orders_id', ['customers_id' => $_SESSION['customer_id']], 'orders_id desc', 1);
...
// - The expiry date is not reached
      if ( ($Qdownloads->valueInt('download_count') > 0) && (is_file(KUUZU::getConfig('dir_root', 'Shop') . 'download/' . $Qdownloads->value('orders_products_filename'))) && ( ($Qdownloads->valueInt('download_maxdays') == 0) || ($download_timestamp > time())) ) {
        echo '        <td><a href="' . KUUZU::link('download.php', 'order=' . $last_order . '&id=' . $Qdownloads->valueInt('orders_products_download_id')) . '">' . $Qdownloads->value('products_name') . '</a></td>' . "\n";
      } else {
...
?>
    ...
    <p><?php printf(FOOTER_DOWNLOAD, '<a href="' . KUUZU::link('account.php') . '">' . KUUZU::getDef('header_title_my_account') . '</a>'); ?></p>
