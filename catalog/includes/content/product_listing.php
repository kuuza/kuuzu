...
<div class="row">
  <div class="col-sm-6 pagenumber hidden-xs">
    <?php echo $Qlisting->getPageSetLabel(KUUZU::getDef('text_display_number_of_products')); ?>
  </div>
  <div class="col-sm-6">
    <div class="pull-right pagenav"><?php echo $Qlisting->getPageSetLinks(tep_get_all_get_params(array('page', 'info', 'x', 'y'))); ?></div>
    <span class="pull-right"><?php echo KUUZU::getDef('text_result_page'); ?></span>
  </div>
</div>
<?php
  }
  ...
  if ($Qlisting->getPageSetTotalRows() > 0) { ?>
    <div class="well well-sm">
      <div class="btn-group btn-group-sm pull-right">
        <button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
          <?php echo KUUZU::getDef('text_sort_by'); ?><span class="caret"></span>
        </button>
        ...
        <ul class="dropdown-menu text-left">
          ...
          for ($col=0, $n=sizeof($column_list); $col<$n; $col++) {
            switch ($column_list[$col]) {
              ...
              $lc_text = KUUZU::getDef('table_heading_model');
              break;
              case 'PRODUCT_LIST_NAME':
              $lc_text = KUUZU::getDef('table_heading_products');
              break;
              case 'PRODUCT_LIST_MANUFACTURER':
              $lc_text = KUUZU::getDef('table_heading_manufacturer');
              break;
              case 'PRODUCT_LIST_PRICE':
              $lc_text = KUUZU::getDef('table_heading_price');
              break;
              case 'PRODUCT_LIST_QUANTITY':
              $lc_text = KUUZU::getDef('table_heading_quantity');
              break;
              case 'PRODUCT_LIST_WEIGHT':
              $lc_text = KUUZU::getDef('table_heading_weight');
              break;
              case 'PRODUCT_LIST_IMAGE':
              $lc_text = KUUZU::getDef('table_heading_image');
              break;
              case 'PRODUCT_LIST_BUY_NOW':
              $lc_text = KUUZU::getDef('table_heading_buy_now');
              break;
              case 'PRODUCT_LIST_ID':
              $lc_text = KUUZU::getDef('table_heading_latest_added');
              break;
            }
    ...
    if ( (defined('MODULE_HEADER_TAGS_GRID_LIST_VIEW_STATUS') && MODULE_HEADER_TAGS_GRID_LIST_VIEW_STATUS == 'True') && (strpos(MODULE_HEADER_TAGS_GRID_LIST_VIEW_PAGES, basename($PHP_SELF)) !== false) ) {
      ?>
      <strong><?php echo KUUZU::getDef('text_view'); ?></strong>
      <div class="btn-group">
        <a href="#" id="list" class="btn btn-default btn-sm"><span class="fa fa-th-list"></span><?php echo KUUZU::getDef('text_view_list'); ?></a>
        <a href="#" id="grid" class="btn btn-default btn-sm"><span class="fa fa-th"></span><?php echo KUUZU::getDef('text_view_grid'); ?></a>
      </div>
      <?php
    }
    ?>
  ...
  while ($Qlisting->fetch()) {
    ...
    if (isset($_GET['manufacturers_id'])  && tep_not_null($_GET['manufacturers_id'])) {
      $prod_list_contents .= '    <a href="' . KUUZU::link('product_info.php', 'manufacturers_id=' . $_GET['manufacturers_id'] . '&products_id=' . $Qlisting->valueInt('products_id')) . '">' . HTML::image(KUUZU::linkImage($Qlisting->value('products_image')), $Qlisting->value('products_name'), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'itemprop="image"', NULL, 'img-responsive thumbnail group list-group-image') . '</a>';
    } else {
      $prod_list_contents .= '    <a href="' . KUUZU::link('product_info.php', ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $Qlisting->valueInt('products_id')) . '">' . HTML::image(KUUZU::linkImage($Qlisting->value('products_image')), $Qlisting->value('products_name'), SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'itemprop="image"', NULL, 'img-responsive thumbnail group list-group-image') . '</a>';
    }
    $prod_list_contents .= '    <div class="caption">';
    $prod_list_contents .= '      <h2 class="group inner list-group-item-heading">';
    if (isset($_GET['manufacturers_id']) && tep_not_null($_GET['manufacturers_id'])) {
      $prod_list_contents .= '    <a itemprop="url" href="' . KUUZU::link('product_info.php', 'manufacturers_id=' . $_GET['manufacturers_id'] . '&products_id=' . $Qlisting->valueInt('products_id')) . '"><span itemprop="name">' . $Qlisting->value('products_name') . '</span></a>';
    } else {
      $prod_list_contents .= '    <a itemprop="url" href="' . KUUZU::link('product_info.php', ($cPath ? 'cPath=' . $cPath . '&' : '') . 'products_id=' . $Qlisting->valueInt('products_id')) . '"><span itemprop="name">' . $Qlisting->value('products_name') . '</span></a>';
    }
    ...
    $extra_list_contents = NULL;
      ...
      $extra_list_contents .= '<dt>' . KUUZU::getDef('table_heading_manufacturer') . '</dt>';
      $extra_list_contents .= '<dd><a href="' . KUUZU::link('index.php', 'manufacturers_id=' . $Qlisting->valueInt('manufacturers_id')) . '">' . $Qlisting->value('manufacturers_name') . '</a></dd>';
    }
	  if ( (PRODUCT_LIST_MODEL > 0) && tep_not_null($Qlisting->value('products_model')) ) {
      $extra_list_contents .= '<dt>' . KUUZU::getDef('table_heading_model') . '</dt>';
      ...
      $extra_list_contents .= '<dt>' . KUUZU::getDef('table_heading_quantity') . '</dt>';
      $extra_list_contents .= '<dd>' . tep_get_products_stock($Qlisting->valueInt('products_id')) . '</dd>';
    }
	  if (PRODUCT_LIST_WEIGHT > 0) {
      $extra_list_contents .= '<dt>' . KUUZU::getDef('table_heading_weight') . '</dt>';
    ...
    if (tep_not_null($extra_list_contents)) {
      ...
      if (PRODUCT_LIST_BUY_NOW > 0) {
        $prod_list_contents .= '       <div class="col-xs-6 text-right">' . HTML::button(KUUZU::getDef('image_button_buy_now'), 'fa fa-shopping-cart', KUUZU::link(basename($PHP_SELF), tep_get_all_get_params(array('action', 'sort', 'cPath')) . 'action=buy_now&products_id=' . $Qlisting->valueInt('products_id')), null, 'btn-success btn-sm btn-product-listing btn-buy') . '</div>';
      }
...
?>
  ...
  <div class="alert alert-info"><?php echo KUUZU::getDef('text_no_products'); ?></div>
