  ...
  */
  ...
  class kuuTemplate {
    ...
    function __construct() {
      ...
      $this->_title = KUUZU::getDef('title', ['store_name' => STORE_NAME]);

      $this->addBlock('<meta name="generator" content="Kuuzu" />', 'header_tags');
    }
    ...
    function getFile($file, $template = null) {
      ...
      return KUUZU::BASE_DIR . 'Sites/' . KUUZU::getSite() . '/Templates/' . $template . '/' . $file;
    }
    ...
    function getPublicFile($file, $template = null) {
      ...
      return KUUZU::linkPublic('Templates/' . $template . '/' . $file);
    }
  }
?>
