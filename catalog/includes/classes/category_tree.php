  ...
  class category_tree {
    ...
    public function __construct() {
      ...
      if ( isset($_category_tree_data) ) {
        ...
        $Qcategories = $KUUZU_Db->prepare('select c.categories_id, c.parent_id, c.categories_image, cd.categories_name from :table_categories c, :table_categories_description cd where c.categories_id = cd.categories_id and cd.language_id = :language_id order by c.parent_id, c.sort_order, cd.categories_name');
        $Qcategories->bindInt(':language_id', $KUUZU_Language->getId());
        $Qcategories->setCache('categories-lang' . $KUUZU_Language->getId());
        $Qcategories->execute();
    ...
    protected function _buildBranch($parent_id, $level = 0) {
      ...
      if ( isset($this->_data[$parent_id]) ) {
        foreach ( $this->_data[$parent_id] as $category_id => $category ) {
          ...
          $result .= '<a href="' . KUUZU::link('index.php', 'cPath=' . $category_link) . '">';
