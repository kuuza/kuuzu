  ...
  */
  ...
  class order {
    ...
    function query($order_id) {
      ...
      $Qorder = $KUUZU_Db->prepare('select * from :table_orders where orders_id = :orders_id');
      $Qorder->bindInt(':orders_id', $order_id);
      $Qorder->execute();

      $Qtotals = $KUUZU_Db->prepare('select title, text, class from :table_orders_total where orders_id = :orders_id order by sort_order');
      ...
      $Qstatus = $KUUZU_Db->prepare('select orders_status_name from :table_orders_status where orders_status_id = :orders_status_id and language_id = :language_id');
      ...
      $Qproducts = $KUUZU_Db->prepare('select orders_products_id, products_id, products_name, products_model, products_price, products_tax, products_quantity, final_price from :table_orders_products where orders_id = :orders_id');
      ...
      while ($Qproducts->fetch()) {
        ...
        $Qattributes = $KUUZU_Db->prepare('select products_options, products_options_values, options_values_price, price_prefix from :table_orders_products_attributes where orders_id = :orders_id and orders_products_id = :orders_products_id');
    ...
    function cart() {
      ...
      if (isset($_SESSION['customer_id'])) {
        $Qcustomer = $KUUZU_Db->prepare('select c.customers_firstname, c.customers_lastname, c.customers_telephone, c.customers_email_address, ab.entry_company, ab.entry_street_address, ab.entry_suburb, ab.entry_postcode, ab.entry_city, ab.entry_zone_id, z.zone_name, co.countries_id, co.countries_name, co.countries_iso_code_2, co.countries_iso_code_3, co.address_format_id, ab.entry_state from :table_customers c, :table_address_book ab left join :table_zones z on (ab.entry_zone_id = z.zone_id) left join :table_countries co on (ab.entry_country_id = co.countries_id) where c.customers_id = :customers_id and c.customers_id = ab.customers_id and c.customers_default_address_id = ab.address_book_id');
      ...
      if (isset($_SESSION['payment'])) {
        ...
        if (isset($KUUZU_PM)) {
          if (isset($KUUZU_PM->public_title)) {
            $this->info['payment_method'] = $KUUZU_PM->public_title;
          } else {
            $this->info['payment_method'] = $KUUZU_PM->title;
          }

          if ( isset($KUUZU_PM->order_status) && is_numeric($KUUZU_PM->order_status) && ($KUUZU_PM->order_status > 0) ) {
            $this->info['order_status'] = $KUUZU_PM->order_status;
          }
        }
      }
