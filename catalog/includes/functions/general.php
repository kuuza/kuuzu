...
// TABLES: products
  function tep_get_products_name($product_id, $language_id = null) {
    ...
    $Qproduct = $KUUZU_Db->prepare('select products_name from :table_products_description where products_id = :products_id and language_id = :language_id');
...
// TABLES: products
  function tep_get_products_special_price($product_id) {
    ...
    $Qproduct = $KUUZU_Db->prepare('select specials_new_products_price from :table_specials where products_id = :products_id and status = 1');
    $Qproduct->bindInt(':products_id', $product_id);
    $Qproduct->execute();
...
// TABLES: products
  function tep_get_products_stock($products_id) {
    ...
    $Qproduct = $KUUZU_Db->prepare('select products_quantity from :table_products where products_id = :products_id');
  ...
  function tep_get_countries($countries_id = '', $with_iso_codes = false) {
    ...
    if (tep_not_null($countries_id)) {
      if ($with_iso_codes == true) {
        $Qcountries = $KUUZU_Db->prepare('select countries_name, countries_iso_code_2, countries_iso_code_3 from :table_countries where countries_id = :countries_id');
        $Qcountries->bindInt(':countries_id', $countries_id);
        $Qcountries->execute();

        $countries_array = $Qcountries->toArray();
      } else {
        $Qcountries = $KUUZU_Db->prepare('select countries_name from :table_countries where countries_id = :countries_id');
        $Qcountries->bindInt(':countries_id', $countries_id);
        $Qcountries->execute();
      ...
      $countries_array = $KUUZU_Db->query('select countries_id, countries_name from :table_countries order by countries_name')->fetchAll();
    }
  ...
  function tep_get_path($current_category_id = '') {
    ...
    if (tep_not_null($current_category_id)) {
      ...
      if ($cp_size == 0) {
        ...
        $QlastCategory = $KUUZU_Db->prepare('select parent_id from :table_categories where categories_id = :categories_id');
        $QlastCategory->bindInt(':categories_id', $cPath_array[($cp_size-1)]);
        $QlastCategory->execute();

        $QcurrentCategory = $KUUZU_Db->prepare('select parent_id from :table_categories where categories_id = :categories_id');
...
// TABLES: zones
  function tep_get_zone_name($country_id, $zone_id, $default_zone) {
    ...
    $Qzone = $KUUZU_Db->prepare('select zone_name from :table_zones where zone_country_id = :zone_country_id and zone_id = :zone_id');
...
// TABLES: zones
  function tep_get_zone_code($country_id, $zone_id, $default_zone) {
    ...
    $Qzone = $KUUZU_Db->prepare('select zone_code from :table_zones where zone_country_id = :zone_country_id and zone_id = :zone_id');
    $Qzone->bindInt(':zone_country_id', $country_id);
    $Qzone->bindInt(':zone_id', $zone_id);
    $Qzone->execute();
  ...
  function tep_get_tax_description($class_id, $country_id, $zone_id) {
    ...
    if (!isset($tax_rates[$class_id][$country_id][$zone_id]['description'])) {
      ...
      if ($Qtax->fetch() !== false) {
        ...
        $tax_rates[$class_id][$country_id][$zone_id]['description'] = KUUZU::getDef('text_unknown_tax_rate');
      }
    }
...
// TABLES: products, products_to_categories, categories
  function tep_count_products_in_category($category_id, $include_inactive = false) {
    ...
    $Qproducts = $KUUZU_Db->prepare($products_query);
    ...
    $Qcategories = $KUUZU_Db->prepare('select categories_id from :table_categories where parent_id = :parent_id');
...
// TABLES: categories
  function tep_has_category_subcategories($category_id) {
    ...
    $Qcheck = $KUUZU_Db->prepare('select categories_id from :table_categories where parent_id = :parent_id limit 1');
...
// TABLES: countries;
  function tep_get_address_format_id($country_id) {
    ...
    $Qformat = $KUUZU_Db->prepare('select address_format_id from :table_countries where countries_id = :countries_id');
...
// TABLES: address_format
  function tep_address_format($address_format_id, $address, $html, $boln, $eoln) {
    ...
    $Qformat = $KUUZU_Db->prepare('select address_format from :table_address_format where address_format_id = :address_format_id');
...
// TABLES: customers, address_book
  function tep_address_label($customers_id, $address_id = 1, $html = false, $boln = '', $eoln = "\n") {
    ...
    $Qaddress = $KUUZU_Db->prepare('select entry_firstname as firstname, entry_lastname as lastname, entry_company as company, entry_street_address as street_address, entry_suburb as suburb, entry_city as city, entry_postcode as postcode, entry_state as state, entry_zone_id as zone_id, entry_country_id as country_id from :table_address_book where address_book_id = :address_book_id and customers_id = :customers_id');
  ...
  function tep_get_categories($categories_array = '', $parent_id = '0', $indent = '') {
    ...
    $Qcategories = $KUUZU_Db->prepare('select c.categories_id, cd.categories_name from :table_categories c, :table_categories_description cd where c.parent_id = :parent_id and c.categories_id = cd.categories_id and cd.language_id = :language_id order by c.sort_order, cd.categories_name');
  ...
  function tep_get_manufacturers($manufacturers_array = '') {
    ...
    $Qmanufacturers = $KUUZU_Db->query('select manufacturers_id, manufacturers_name from :table_manufacturers order by manufacturers_name');
...
// TABLES: categories
  function tep_get_subcategories(&$subcategories_array, $parent_id = 0) {
    ...
    $Qsub = $KUUZU_Db->prepare('select categories_id from :table_categories where parent_id = :parent_id');
...
// Return table heading with sorting capabilities
  function tep_create_sort_heading($sortby, $colnum, $heading) {
    ...
    if ($sortby) {
      $sort_prefix = '<a href="' . KUUZU::link($PHP_SELF, tep_get_all_get_params(array('page', 'info', 'sort')) . 'page=1&sort=' . $colnum . ($sortby == $colnum . 'a' ? 'd' : 'a')) . '" title="' . HTML::output(KUUZU::getDef('text_sort_products') . ($sortby == $colnum . 'd' || substr($sortby, 0, 1) != $colnum ? KUUZU::getDef('text_ascendingly') : KUUZU::getDef('text_descendingly')) . KUUZU::getDef('text_by') . $heading) . '" class="productListing-heading">' ;
...
// TABLES: categories
  function tep_get_parent_categories(&$categories, $categories_id) {
    ...
    $Qparent = $KUUZU_Db->prepare('select parent_id from :table_categories where categories_id = :categories_id');
...
// TABLES: products_to_categories
  function tep_get_product_path($products_id) {
    ...
    $Qcategory = $KUUZU_Db->prepare('select p2c.categories_id from :table_products p, :table_products_to_categories p2c where p.products_id = :products_id and p.products_status = 1 and p.products_id = p2c.products_id limit 1');
...
// Check if product has attributes
  function tep_has_product_attributes($products_id) {
    ...
    $Qattributes = $KUUZU_Db->prepare('select products_id from :table_products_attributes where products_id = :products_id limit 1');
    ...
    for ($i=0, $n=sizeof($modules_array); $i<$n; $i++) {
      ...
      if (strpos($m, '\\') !== false) {
        ...
        if (Registry::exists($code)) {
          $KUUZU_PM = Registry::get($code);
        }
      } else {
        $module = substr($m, 0, strrpos($m, '.'));

        if (is_object($GLOBALS[$module])) {
          $KUUZU_PM = $GLOBALS[$module];
        }
      }

      if (isset($KUUZU_PM) && $KUUZU_PM->enabled) {
        $count++;
      }
    }
  ...
  function tep_count_shipping_modules() {
    ...
    for ($i=0, $n=sizeof($modules_array); $i<$n; $i++) {
      ...
      if (strpos($m, '\\') !== false) {
        ...
        if (Registry::exists($code)) {
          $KUUZU_SM = Registry::get($code);
        }
      } else {
        $module = substr($m, 0, strrpos($m, '.'));

        if (is_object($GLOBALS[$module])) {
          $KUUZU_SM = $GLOBALS[$module];
        }
      }

      if (isset($KUUZU_SM) && $KUUZU_SM->enabled) {
        $count++;
      }
    }
...
// TABLES: currencies
  function tep_currency_exists($code) {
    ...
    $Qcurrency = $KUUZU_Db->prepare('select code from :table_currencies where code = :code limit 1');
...
// make sure no duplicate category IDs exist which could lock the server in a loop
  ...
  function tep_count_customer_orders($id = '', $check_session = true) {
    ...
    $Qorders = $KUUZU_Db->prepare('select count(*) as total from :table_orders o, :table_orders_status s where o.customers_id = :customers_id and o.orders_status = s.orders_status_id and s.language_id = :language_id and s.public_flag = 1');
  ...
  function tep_count_customer_address_book_entries($id = '', $check_session = true) {
    ...
    $Qaddresses = $KUUZU_Db->prepare('select count(*) as total from :table_address_book where customers_id = :customers_id');
...
// Creates a pull-down list of countries
  function tep_get_country_list($name, $selected = '', $parameters = '') {
    $countries_array = array(array('id' => '', 'text' => KUUZU::getDef('pull_down_default')));
