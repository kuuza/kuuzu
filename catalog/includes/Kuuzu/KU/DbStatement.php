    ...
    public function execute($input_parameters = null)
    {
        ...
        if ($this->cache_read === false) {
            ...
            if (strpos($this->queryString, ' SQL_CALC_FOUND_ROWS ') !== false) {
                ...
                trigger_error('Kuuzu\KU\DbStatement::execute(): Page Set query does not contain SQL_CALC_FOUND_ROWS. Please add it to the query: ' . $this->queryString);
            }
        }
    }
...
// use the optional argument
    ...
    public function setCache($key, $expire = null, $cache_empty_results = false)
    {
        ...
        if ($this->query_call != 'prepare') {
            trigger_error('Kuuzu\\KU\\DbStatement::setCache(): Cannot set cache (\'' . $key . '\') on a non-prepare query. Please change the query to a prepare() query.');
        }
    }
    ...
    public function getPageSetLinks($parameters = null)
    {
        ...
        if ($number_of_pages > 1) {
            $output .= '<li>' . HTML::selectField('pageset' . $this->page_set_keyword, $pages, $this->page_set, 'style="vertical-align: top; display: inline-block; float: left; width: 80px;" data-pageseturl="' . HTML::output(KUUZU::link($PHP_SELF, $parameters . $this->page_set_keyword . '=PAGESETGOTO')) . '"') . '</li>';
        } else {
...
// previous button
        if ($this->page_set > 1) {
            $output .= '<li><a href="' . KUUZU::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . ($this->page_set - 1)) . '" title="' . KUUZU::getDef('prevnext_title_previous_page') . '" class="text-center" style="width: 80px;"><span class="fa fa-fw fa-chevron-left"></span></a></li>';
        } else {
...
// next button
        if (($this->page_set < $number_of_pages) && ($number_of_pages != 1)) {
            $output .= '<li><a href="' . KUUZU::link($PHP_SELF, $parameters . $this->page_set_keyword . '=' . ($this->page_set + 1)) . '" title="' . KUUZU::getDef('prevnext_title_next_page') . '" class="text-center" style="width: 80px;"><span class="fa fa-fw fa-chevron-right"></span></a></li>';
        } else {
