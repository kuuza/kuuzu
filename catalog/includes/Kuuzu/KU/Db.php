...
class Db extends \PDO
{
    ...
    public static function initialize(
        ...
        if (!isset($driver_options[\PDO::ATTR_STATEMENT_CLASS])) {
            $driver_options[\PDO::ATTR_STATEMENT_CLASS] = array('Kuuzu\KU\DbStatement');
        }
        ...
        try {
            ...
            if (!isset($options['log_errors']) || ($options['log_errors'] === true)) {
                error_log('Kuuzu\KU\Db::initialize(): ' . $message);
            }
    ...
    public function importSQL($sql_file, $table_prefix = null)
    {
        if (is_file($sql_file)) {
            ...
            trigger_error('Kuuzu\KU\Db::importSQL(): SQL file does not exist: ' . $sql_file);
...
// join the query before the comment appeared, with the rest of the dump
    ...
    protected function autoPrefixTables($statement)
    {
        ...
        } elseif (KUUZU::configExists('db_table_prefix')) {
            $prefix = KUUZU::getConfig('db_table_prefix');
        }
