...
class HTTP
{
    ...
    public static function getResponse(array $parameters)
    {
        ...
        if ($parameters['server']['scheme'] == 'https') {
            $verify_ssl = (defined('KUUZU_HTTP_VERIFY_SSL') && (KUUZU_HTTP_VERIFY_SSL === 'True')) ? true : false;
            ...
            if (!isset($parameters['cafile'])) {
                $parameters['cafile'] = KUUZU::getConfig('dir_root', 'Shop') . 'includes/cacert.pem';
            }
        ...
        $proxy = defined('KUUZU_HTTP_PROXY') ? KUUZU_HTTP_PROXY : '';
