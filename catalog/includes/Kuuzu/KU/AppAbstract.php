...
abstract class AppAbstract
{
    ...
    final private function setInfo()
    {
        ...
        if (!is_file($metafile) || (($json = json_decode(file_get_contents($metafile), true)) === null)) {
            trigger_error('Kuuzu\KU\AppAbstract::setInfo(): ' . $this->vendor . '\\' . $this->code . ' - Could not read App information in ' . $metafile . '.');
    ...
    final public function definitionsExist($group, $language_code = null)
    {
        ...
        $pathname = KUUZU::BASE_DIR . 'Apps/' . $this->vendor . '/' . $this->code . '/languages/' . $this->lang->get('directory', $language_code) . '/' . $group . '.txt';
