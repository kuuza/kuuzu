...
class Hash
{
    public static function encrypt($plain, $algo = null)
    {
        ...
        if ($algo == 'phpass') {
            if (!class_exists('PasswordHash', false)) {
                include(KUUZU::getConfig('dir_root', 'Shop') . 'includes/third_party/PasswordHash.php');
            }
        ...
        trigger_error('Kuuzu\\KU\\Hash::encrypt() Algorithm "' . $algo . '" unknown.');
    ...
    public static function needsRehash($hash, $algo = null)
    {
        ...
        if (!is_int($algo)) {
            trigger_error('Kuuzu\KU\Hash::needsRehash() Algorithm "' . $algo . '" not supported.');
        }
    ...
    public static function getType($hash)
    {
        ...
        trigger_error('Kuuzu\KU\Hash::getType() hash type not found for "' . substr($hash, 0, 5) . '"');
