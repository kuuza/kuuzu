...
class Language
{
    ...
    public function set($code)
    {
        if ($this->exists($code)) {
            ...
            trigger_error('Kuuzu\KU\Language::set() - The language does not exist: ' . $code);
        }
    }
    ...
    public function getImage($language_code, $width = null, $height = null)
    {
        ...
        return HTML::image(KUUZU::link('Shop/public/third_party/flag-icon-css/flags/4x3/' . $this->get('image', $language_code) . '.svg', null, false), $this->get('name', $language_code), $width, $height);
    }
    ...
    public function definitionsExist($group, $language_code = null)
    {
        ...
        if ((strpos($group, '/') !== false) && (preg_match('/^([A-Z][A-Za-z0-9-_]*)\/(.*)$/', $group, $matches) === 1) && KUUZU::siteExists($matches[1])) {
            $site = $matches[1];
            $group = $matches[2];
        }

        $pathname = KUUZU::getConfig('dir_root', $site) . 'includes/languages/' . $this->get('directory', $language_code) . '/' . $group;
