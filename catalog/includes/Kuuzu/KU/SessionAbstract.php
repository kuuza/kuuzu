 ...
 */
    ...
    {
        $KUUZU_Cookies = Registry::get('Cookies');
...
// this class handles session.use_strict_mode already
        ...
        if (parse_url(KUUZU::getConfig('http_server'), PHP_URL_SCHEME) == 'https') {
        ...
        session_set_cookie_params(0, $KUUZU_Cookies->getPath(), $KUUZU_Cookies->getDomain(), (bool)ini_get('session.cookie_secure'), (bool)ini_get('session.cookie_httponly'));
        ...
        if (isset($_COOKIE[$this->name]) && (!(bool)preg_match('/^[a-zA-Z0-9,-]+$/', $_COOKIE[$this->name]) || !$this->exists($_COOKIE[$this->name]))) {
            $KUUZU_Cookies->del($this->name, $KUUZU_Cookies->getPath(), $KUUZU_Cookies->getDomain(), (bool)ini_get('session.cookie_secure'), (bool)ini_get('session.cookie_httponly'));
        }
